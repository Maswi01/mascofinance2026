...
def index(request):
    ...
    # ── Per-office summary cards (index page special rule) ────────────────────
    # If the logged-in user's office_allocation = HQ → always show ALL branches
    # regardless of which office they have selected in the dropdown.
    # For all other users → show only their selected/allocated office.
    user_primary_office = getattr(request.user, 'office_allocation', None)
    ...
    if is_hq_allocated:
        # HQ staff always see all branches on the home page
        target_offices = Office.objects.all()
    elif filter_office:
        # Branch staff see only their currently selected office
        ...
        target_offices = Office.objects.filter(pk=user_primary_office.pk)
    else:
        target_offices = Office.objects.none()

    # target_offices = target_offices.order_by('name')
    target_offices = target_offices.exclude(name__iexact='HQ').order_by('name')
    ...
    for office in target_offices:
        ...
        r = LoanRepayment.objects.filter(
            loan_application__office__iexact=office.name,  # ← iexact instead of exact
            created_at__date=display_date,
    ...
    # Check if office filter is the problem
    reps = LoanRepayment.objects.filter(created_at__date=today)
    print("Total today:", reps.count())

    for r in reps[:5]:
        print(r.loan_application.office, '|', r.repayment_amount)
...
def loan_repayment_schedule(request, loan_id):
    ...
    # ── Build a lookup: (year, month) → total paid that month ──────────
    paid_by_month = {}
    for r in repayments:
        if r.payment_month:
    ...
    for i in range(periods):
        ...
        # ── Look up paid amount by payment_month, not by index ──────────
...
def monthly_repayment_report(request):
    ...
    for (year, month) in period_months:
        ...
        # Filtered by payment_month (DateField) instead of repayment_date
        # so records appear in the month they were allocated to, not
        # necessarily the date the payment was physically recorded.
        repayments = LoanRepayment.objects.filter(
        ...
        for rep in repayments:
            ...
            raw.append({
                ...
                'date':          rep.repayment_date,   # display still uses repayment_date
...
def _distribute_payments(col_months, rmap=None, inst=Decimal('0'), frd=None,
    ...
    • Malipo nje ya period → yaonyeshwe (paid + balance iliyobaki globally).
    ─────────────────────────────────────────────────────────────────
    """
    rmap  = rmap or {}
    today = datetime.date.today()
    ...
    # ── 3. TOPUP INFO ─────────────────────────────────────────────────────────
    # topup_lump  = old_balance_cleared = kiasi kilichofunika mkopo wa zamani
    # topup_month = mwezi wa topup
    ...
    # Topup inalipwa kwa mkopo mpya (si cash) — rmap entry ya topup_month
    # ni duplicate ya topup_lump. Tumia topup_lump peke yake kwa mwezi huo.
    ...
    # Gawanya clean_rmap kwenye slots (topup_month imeondolewa tayari)
    ...
    # Gawanya topup_lump kwenye slots (old_balance_cleared)
    if topup_month is not None and topup_lump > 0:
    ...
    cells                   = []
    col_totals_contribution = []
    first_rmap_ym           = min(clean_rmap.keys(), default=None)
    ...
    for (cy, cm) in col_months:
        ...
        # Balance kabla na baada ya mwezi huu
        prev_months = [
            (int(py), int(pm))
        ...
        if 0 <= offset < period:
            ...
            # ── paid_display ──────────────────────────────────────────────────
            # Topup month → onyesha topup_lump (old_balance_cleared = 50k)
            # rmap ya mwezi huu → onyesha hiyo (rmap_this_month)
            # Spillover → paid = 0 (tick tu, paid ilionyeshwa mwezi wa kweli)

            if (topup_month is not None and (cy, cm) == topup_month):
                # Onyesha old_balance_cleared tu (rmap_this_month = 0 tayari)
        ...
        # ── NJE YA PERIOD ─────────────────────────────────────────────────────
            ...
            if paid_this_month > 0:
                if balance_this <= Decimal('0') or loan_fully_paid:
                    ...
                    col_totals_contribution.append((rmap_this_month, balance_this))
...
def _distribute_payments_hama(col_months, rmap, inst, frd, period, cur_y, cur_m, bf_y, bf_m):
    ...
    out_brought_forward = max(total_inst_sum - paid_upto_dec, Decimal('0'))
...
def loans_owed_report(request):
    ...
    hama_cutoff = today - relativedelta(months=12)
    ...
    following_cols = _months_range(cur_y, 1, cur_y, cur_m)
    hama_col_months = [(bf_y, bf_m)] + following_cols
