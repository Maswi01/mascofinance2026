...
def switch_branch(request):
    """Saves selected office to session and redirects back to previous page."""
    ...
    if office_id:
        ...
        request.session['selected_office_id'] = None
        request.session['selected_office_name'] = 'All Branches'
...
def get_selected_office(request):
    """
    Returns the office the current user is filtered to.
    - Superuser: whatever is saved in session (None = All Branches)
    - Regular user: always their own office_allocation
    """
    if request.user.is_superuser:
        ...
        return getattr(request.user, 'office_allocation', None)
...
def index(request):
    ...
    # ── Get selected office from session (set by switch_branch view) ──────
    selected_office = get_selected_office(request)  # ← only this, remove all the old GET param logic below it

    # ── Global stats (scoped to selected office) ───────────────────────────
    def filter_loans(office=None):
        qs = LoanApplication.objects.all()
        if office:
            qs = qs.filter(processed_by__office_allocation=office)
        return qs

    def filter_repayments(office=None):
        qs = LoanRepayment.objects.all()
        if office:
            qs = qs.filter(processed_by__office_allocation=office)
        return qs

    def filter_expenses(office=None):
        qs = Expense.objects.all()
        if office:
            qs = qs.filter(
                Q(office=office.name) | Q(recorded_by__office_allocation=office)
            )
        return qs

    total_repayments = filter_repayments(selected_office).aggregate(
        total=Sum('repayment_amount'))['total'] or Decimal('0.00')

    total_clients = Client.objects.all()
    if selected_office:
        total_clients = total_clients.filter(
            loan_applications__processed_by__office_allocation=selected_office
        ).distinct()
    total_clients = total_clients.count()

    total_due_loan = filter_loans(selected_office).aggregate(
        total=Sum('repayment_amount_remaining'))['total'] or Decimal('0.00')

    repayment_rate = (
        (total_due_loan / (total_repayments + total_due_loan)) * Decimal('100')
    ) if (total_repayments + total_due_loan) > 0 else Decimal('0.00')
    repayment_rate = repayment_rate.quantize(Decimal('0.01'))

    # ── Today's per-office breakdown ───────────────────────────────────────
    target_offices = [selected_office] if selected_office else list(offices)

    today_loans_by_office = []
    today_expenses_by_office = []
    today_repayments_by_office = []
    today_transactions_by_office = []

    for office in target_offices:
        r = LoanApplication.objects.filter(
            processed_by__office_allocation=office,
            application_date=today
        ).aggregate(total_amount=Sum('loan_amount'), count=Count('id'))
        today_loans_by_office.append({
            'office': office.name,
        ...
        today_expenses_by_office.append({
            'office': office.name,
        ...
        today_repayments_by_office.append({
            'office': office.name,
            'amount': r['total_amount'] or Decimal('0.00'),
            'count': r['count'] or 0,
        })
        ...
        to_ = OfficeTransaction.objects.filter(
            office_to=office, transaction_date=today
        ).aggregate(total_amount=Sum('amount'), count=Count('id'))
        fr_ = OfficeTransaction.objects.filter(
            office_from=office, transaction_date=today
        ).aggregate(total_amount=Sum('amount'), count=Count('id'))
        today_transactions_by_office.append({
            'office': office.name,
    ...
    context = {
        ...
        'today_loans_by_office': today_loans_by_office,
        'today_expenses_by_office': today_expenses_by_office,
        'today_repayments_by_office': today_repayments_by_office,
        'today_transactions_by_office': today_transactions_by_office,
        'today': today,
        'offices': offices,
...
def staff_list(request):
    """Page 1: Staff List - Shows all staff in a table"""
...
def customer_plan_calculator(request):
    ...
    return render(request, 'app/customer_plan_calculator.html')
...
def client_add(request):
    ...
    return render(request, 'app/clients.html', {'form': form})

def client_edit(request, pk):
    """
    Called by the edit modal form (POST).
    After saving, redirects back to the client list.
    GET request simply redirects to the list (editing happens in the popup).
    """
    client = get_object_or_404(Client, pk=pk)

    if request.method == 'POST':
        try:
            client.firstname           = request.POST.get('firstname', '').strip()
            client.middlename          = request.POST.get('middlename', '').strip()
            client.lastname            = request.POST.get('lastname', '').strip()
            client.phonenumber         = request.POST.get('phonenumber', '').strip()
            client.date_of_birth       = request.POST.get('date_of_birth') or None
            client.marital_status      = request.POST.get('marital_status', '').strip()
            client.employername        = request.POST.get('employername', '').strip()
            client.idara               = request.POST.get('idara', '').strip()
            client.kaziyako            = request.POST.get('kaziyako', '').strip()
            client.checkno             = request.POST.get('checkno', '').strip()
            client.employmentcardno    = request.POST.get('employmentcardno', '').strip()
            client.tareheya_kuajiriwa  = request.POST.get('tareheya_kuajiriwa') or None
            client.umri_kazini         = request.POST.get('umri_kazini') or None
            client.tarehe_ya_kustaafu  = request.POST.get('tarehe_ya_kustaafu') or None
            client.region              = request.POST.get('region', '').strip()
            client.district            = request.POST.get('district', '').strip()
            client.street              = request.POST.get('street', '').strip()
            client.mkoa                = request.POST.get('mkoa', '').strip()
            client.wilaya              = request.POST.get('wilaya', '').strip()
            client.tarafa              = request.POST.get('tarafa', '').strip()
            client.kata                = request.POST.get('kata', '').strip()
            client.mtaa                = request.POST.get('mtaa', '').strip()
            client.wategemezi_wako     = request.POST.get('wategemezi_wako', '').strip()
            ...
            client.account_name        = request.POST.get('account_name', '').strip()
            client.account_type        = request.POST.get('account_type', 'savings').strip()
            client.save()
            messages.success(
                request,
                f'Customer {client.firstname} {client.lastname} updated successfully.'
            )
...
def office_transaction_add(request):
    if request.method == 'POST':
        ...
        office_from = request.user.office_allocation
...
def loans(request):
    ...
    if selected_office:
        loans = loans.filter(processed_by__office_allocation=selected_office)
...
def calculate_installment_schedule(loan):
    """
    Calculate the monthly installment schedule for the loan
    """
    schedule = []
    
    # Get loan details
    total_principal = Decimal(str(loan.loan_amount))
    interest_rate = Decimal(str(loan.interest_rate)) / Decimal('100')
    months = loan.payment_period_months
    
    # Calculate monthly values
    monthly_principal = total_principal / Decimal(months)
    monthly_interest_total = loan.total_interest_amount / Decimal(months)
    
    current_balance = loan.total_repayment_amount
    current_date = loan.first_repayment_date
    
    for i in range(1, months + 1):
        # Calculate values for this installment
        if i == months:  # Last installment
            monthly_payment = current_balance
        else:
            monthly_payment = monthly_principal + monthly_interest_total
        
        # Update balance
        remaining_balance = current_balance - monthly_payment if i < months else Decimal('0.00')
        ...
        schedule.append({
            ...
            'interest': monthly_interest_total,
            'total_payment': monthly_payment,
            'remaining_balance': remaining_balance,
            'is_current': i == 1  # Mark first as current (you can customize this logic)
        })
    ...
    if request.method == 'POST':
        ...
        try:
            with transaction.atomic():
                topup_amount = Decimal(request.POST.get('topup_amount'))
                ...
                if topup_amount <= outstanding:
                    messages.error(
                        request, 
                        f"Topup amount ({topup_amount}) must be greater than outstanding balance ({outstanding})"
                    )
                    return redirect('loans')
                
                # Calculate net disbursement (amount actually sent to client)
                ...
                # Check sufficient balance & deduct net disbursement based on transaction method
                if transaction_method == 'cash':
                ...
                # Create new loan with topup amount
                new_loan = LoanApplication.objects.create(
                    client=loan.client,
                    loan_amount=topup_amount,
                    loan_purpose=f"Topup of loan #{loan.id} - {loan.loan_purpose}",
                ...
                # Save LoanTopup record
                LoanTopup.objects.create(
                    loan_application=new_loan,
                ...
                messages.success(
                    request, 
                    f"Topup completed successfully! New loan #{new_loan.id} created. "
                    f"Client receives: {net_disbursement}"
                )
...
def expense_add(request):
    if request.method == 'POST':
        try:
            ...
            branch_office = request.user.office_allocation
            if not branch_office:
                messages.error(request, 'You are not allocated to any office/branch.')
...
def expense_report(request):
    ...
    return render(request, 'app/expense_report.html', context)
...
def loans_owed_report(request):
    ...
    Builds hama_rows: clients with any outstanding/partial month (for HAMA table).
    """
...
# nyongeza is just like add capital
def nyongeza(request):
    offices = Office.objects.all()
    # 👇 DELETE this line — it conflicts with the block below
    # selected_office = get_selected_office(request)  
    ...
    if request.user.is_superuser:
        ...
        if office_id:
            try:
                selected_office = Office.objects.get(id=office_id)
            except Office.DoesNotExist:
                selected_office = None
    else:
        selected_office = getattr(request.user, 'office_allocation', None)


    # ── Filter Nyongeza by office ─────────────────────────────────────────
    nyongeza_qs = Nyongeza.objects.all().order_by('-id')

    if selected_office:
        nyongeza_qs = nyongeza_qs.filter(
            recorded_by__office_allocation=selected_office
        )
    ...
    # ── Unique dates and users for filter dropdowns ───────────────────────
    ...
    return render(request, 'app/nyongeza.html', context)
...
def nyongeza_add(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # Get the user's office allocation
            user_office = request.user.office_allocation
            if not user_office:
                messages.error(request, 'You are not allocated to any office/branch.')
                return redirect('nyongeza')
            
            # Get or create branch balance for the user's office
            branch_balance = BranchBalance.objects.filter(branch=user_office).last()
            
            if not branch_balance:
                # Initialize with amount in the correct balance based on deposit_method
                BranchBalance.objects.create(
                    branch=user_office,
                ...
                # Add to correct balance based on deposit_method
            ...
            # Create Nyongeza record linked to user's office
...
def office_transaction_page(request):
    return render(request, 'app/office_transaction_page.html')
...
def process_loan_partb(request, pk):
    ...
    2. Renders Part B (loan details form), passing the client and any existing active loan.
    """
    client = get_object_or_404(Client, pk=pk)
...
def loan_application(request):
    """
    Handles Part B form submission.
    Creates the LoanApplication, deducts from branch balance, redirects to receipt.
    """
    if request.method == 'POST':
        client_id          = request.POST.get('client')
        loan_amount        = request.POST.get('amount')
        ...
        topup_date         = request.POST.get('topup_date') or None
        processed_by       = request.user
        ...
        try:
            ...
            if existing_loan:
                messages.error(
                    request,
                    f"Client {client} has an existing pending loan application. "
                    "Cannot apply for a new loan until the existing one is processed."
                )
            ...
            messages.error(request, f'Error processing loan application: {str(e)}')
            client_id_safe = client_id if client_id else None
            if client_id_safe:
                return redirect('process_loan_partb', pk=int(client_id_safe))

    # GET — redirect back to client list
...
def loan_payment_page(request):
    """
    Page 1: Searchable dropdown to select a customer with an active loan.
    Shows all loans that are NOT fully paid (status != 'Paid').
    """
...
def loan_repayment(request, loan_id):
    """
    Page 2: Repayment form for a specific loan.
    GET  → show the form with repayment history.
    ...
    # Generate month options from first repayment date to today
    repayment_months = []
    today = datetime.date.today()

    start_date = loan.first_repayment_date or loan.application_date
    # Generate from start_date up to current month
    ...
    if request.method == 'POST':
        repayment_amount_raw = request.POST.get('repayment_amount')
        transaction_method   = request.POST.get('transaction_method', 'cash')
        processed_by         = request.user
...
def bank_cash_transaction_add(request):
    """
    Handles: Transfer to Bank / Cash
    Source is auto-derived as the opposite of the selected destination.
    Office is always taken from the logged-in user's office_allocation.
    """
    processed_by   = request.user
    user_office    = getattr(processed_by, 'office_allocation', None)
    branch_balance = BranchBalance.objects.filter(branch=user_office).last() if user_office else None
    offices        = Office.objects.all()

    if request.method == 'POST':
        try:
            source      = request.POST.get('source')       # injected by JS (opposite of destination)
            destination = request.POST.get('destination')  # bank or cash
            amount      = Decimal(request.POST.get('amount'))

            office_from = user_office
            if not office_from:
                messages.error(request, 'You are not assigned to any branch office.')
                return redirect('bank_cash_transaction')

            branch_balance = BranchBalance.objects.filter(branch=office_from).last()
            if not branch_balance:
            ...
            BankCashTransaction.objects.create(
                office_from=office_from,
                source=source,
            ...
            messages.success(
                request,
                f'Transfer of TZS {amount:,.0f} from {source.title()} to {destination.title()} completed.'
            )
...
def branches_transaction_add(request):
    ...
    Deducts amount from sending office cash or bank balance.
    office_from is always the logged-in user's office_allocation.
    """
    processed_by   = request.user
    user_office    = getattr(processed_by, 'office_allocation', None)
    offices        = Office.objects.all()
    ...
    if request.method == 'POST':
        ...
        transaction_method = request.POST.get('transaction_method', 'cash')  # cash or bank
        amount             = Decimal(request.POST.get('amount'))

        office_from = user_office
        if not office_from:
            messages.error(request, 'You are not assigned to any office.')
            return redirect('bank_cash_transaction')

        office_to = get_object_or_404(Office, id=office_to_id)

        # ── Validate and deduct from branch balance ───────────────────
        branch_balance = BranchBalance.objects.filter(branch=office_from).last()
        if not branch_balance:
            messages.error(request, f'No balance record found for {office_from.name}.')
            return redirect('bank_cash_transaction')
        ...
        if transaction_method == 'cash':
            if branch_balance.office_balance < amount:
                messages.error(request, f'Insufficient cash balance. Available: TZS {branch_balance.office_balance:,.0f}')
                return redirect('bank_cash_transaction')
            new_office_balance -= amount
        else:  # bank
            if branch_balance.bank_balance < amount:
        ...
        # ── Record the inter-office transaction ───────────────────────
        OfficeTransaction.objects.create(
            office_from=office_from,
            office_to=office_to,
            transaction_type=transaction_type,
        ...
        messages.success(
            request,
            f'Transfer of TZS {amount:,.0f} ({transaction_method.title()}) to {office_to.name} completed successfully.'
        )
        return redirect('bank_cash_transaction')
...
def loan_receipt_select(request):
    """
    Page 1 — Searchable customer dropdown.
    All clients who have at least one loan repayment are shown.
    Submits to loan_receipt_list (POST).
    """
    # Show all clients that have at least one repayment recorded
...
def loan_receipt_list(request):
    """
    Page 2 — List of all repayments for the selected client.
    Handles both POST (from Page 1 / filter form) and GET (with ?client_id=).
    """
    # Resolve client_id from POST or GET
    client_id = request.POST.get('client_id') or request.GET.get('client_id')
    ...
    # All clients for the filter dropdown
    all_clients = Client.objects.filter(
        loan_applications__repayments__isnull=False
    ).distinct().order_by('firstname', 'lastname')

    # Gather all repayments for this client across all loans
    ...
    # Attach sequential receipt numbers (global, padded to 6 digits)
    # Receipt number = base offset + repayment pk
    # Using the pk directly formatted gives stable, unique receipt numbers
    repayment_rows = []
    for r in repayments_qs:
        r.receipt_number = str(r.id).zfill(6)
        repayment_rows.append(r)

    # Determine branch name from the most recent loan's office field
    branch_name = '—'
...
def loan_repayment_receipt(request, pk):
    """
    Page 3 — Individual repayment receipt (STAKABADHI YA FEDHA).
    Displays the Masco Finance receipt for a single LoanRepayment record.
    """
    ...
    # Receipt number: zero-padded pk (matches what's shown in the list)
    ...
    # Payment month label: "Mmm/YYYY" in Swahili months or English
    repayment_date = repayment.repayment_date
    swahili_months = {
        1: 'Jan', 2: 'Feb', 3: 'Mar', 4: 'Apr', 5: 'Mei', 6: 'Jun',
        7: 'Jul', 8: 'Ago', 9: 'Sep', 10: 'Okt', 11: 'Nov', 12: 'Des',
    }
    payment_month = f"{swahili_months.get(repayment_date.month, repayment_date.month)}/{repayment_date.year}"

    # Balance AFTER this repayment
    # Sum all repayments for this loan up to and including this one
    ...
    balance_after = (loan.total_repayment_amount or Decimal('0.00')) - paid_up_to_this
    balance_after = max(balance_after, Decimal('0.00'))

    # Officer name: processed_by full name + username fallback
    officer_user = repayment.processed_by
    officer_name = (
        f"{officer_user.get_full_name()}".strip()
        if officer_user.get_full_name()
        else officer_user.username
    ).upper()
...
def loan_outstanding_select(request):
    """
    Page 1 — Searchable customer selector.
    Shows all clients who have at least one loan application.
    Auto-submits on selection to loan_outstanding_report.
    """
    clients = Client.objects.filter(
...
def loan_outstanding_report(request):
    """
    Page 2 — All loans for the selected client with:
    Loan Type | Loan ID | Loan Amount | Interest | Penalty | Total | Paid | Outstanding Balance
    Each row has a "View" button → loan_repayment_schedule.
    Handles both POST (from selector / filter) and GET (with ?client_id=).
    """
    client_id = request.POST.get('client_id') or request.GET.get('client_id')
    ...
    # Determine branch from most recent loan office field
    branch_name = '—'
    latest_loan = loans.order_by('-created_at').first()
    if latest_loan and latest_loan.office:
        branch_name = latest_loan.office.upper()

    # Build row data for each loan
    loan_rows = []
    for loan in loans:
        # Total paid = sum of all repayments for this loan
        paid_amount = sum(
            r.repayment_amount for r in loan.repayments.all()
        )

        loan_amount    = loan.loan_amount or Decimal('0.00')
        interest_total = loan.total_interest_amount or Decimal('0.00')
        penalty        = Decimal('0.00')   # no penalty model field; extend if needed
        total_amount   = loan.total_repayment_amount or Decimal('0.00')
        outstanding    = max(loan.repayment_amount_remaining, Decimal('0.00'))

        # Loan ID label: "{office}-{id}"
...
def loan_repayment_schedule(request, loan_id):
    """
    Page 3 — Repayment schedule for a single loan.

    The schedule has `payment_period_months` rows.
    Each row = one installment month.

    Actual repayments are matched to schedule rows in ORDER:
    - repayment[0]  → schedule row[0]
    - repayment[1]  → schedule row[1]
    - ...
    Rows beyond the number of actual repayments have Paid = 0 and Out = total.

    Principal and Interest per installment are split proportionally:
      - principal_per_month = loan_amount / payment_period_months
      - interest_per_month  = total_interest_amount / payment_period_months
    ...
    # All actual repayments for this loan, ordered oldest first
    ...
    total_repay  = loan.total_repayment_amount or (loan_amount + total_int)

    # Monthly breakdown
    principal_per = (loan_amount / periods).quantize(Decimal('0.01'))
    interest_per  = (total_int   / periods).quantize(Decimal('0.01'))
    monthly_total = (total_repay / periods).quantize(Decimal('0.01'))

    # Swahili/English month abbreviations
    month_names = {
        1:'Jan', 2:'Feb', 3:'Mar', 4:'Apr', 5:'May', 6:'Jun',
        7:'Jul', 8:'Aug', 9:'Sep', 10:'Oct', 11:'Nov', 12:'Dec'
    }

    # Generate month labels starting from first_repayment_date
    ...
    for i in range(periods):
        # Calculate the month for this row
        month_offset = i
        year  = start.year  + (start.month - 1 + month_offset) // 12
        month = (start.month - 1 + month_offset) % 12 + 1
        month_label = f"{month_names[month]}/{year}"

        # Last row absorbs rounding remainders
        if i == periods - 1:
            p = loan_amount  - principal_per * (periods - 1)
            n = total_int    - interest_per  * (periods - 1)
            t = total_repay  - monthly_total * (periods - 1)
        else:
            p = principal_per
            n = interest_per
            t = monthly_total
        ...
        # Match actual repayment if available
        if i < len(repayments):
            paid = repayments[i].repayment_amount
        else:
            paid = Decimal('0.00')
...
def customer_statement_select(request):
    ...
    Page 1 — Searchable customer dropdown.
    Shows all clients who have at least one loan application.
    Auto-submits to customer_statement on selection.
    """
    clients = Client.objects.filter(
        loan_applications__isnull=False
    ).distinct().order_by('firstname', 'lastname')
...
def customer_statement(request):
    """
    Page 2 — Full chronological transaction statement for a client.

    For every loan the client has, we build a flat list of entries:
      1. One DISBURSEMENT entry:
         - Date        = loan.application_date
         - Receipt No  = zero-padded loan.id  (e.g. 000122)
         - Description = "Loan Taken and interest"
         - Loan col    = total_repayment_amount (principal + interest)
         - Paid col    = blank
         - Balance     = total_repayment_amount  (running balance starts here)

      2. One REPAYMENT entry per LoanRepayment, in date order:
         - Date        = repayment.repayment_date
         - Receipt No  = zero-padded repayment.id (e.g. 000194)
         - Description = "Loan payment"
         - Loan col    = blank
         - Paid col    = repayment.repayment_amount
         - Balance     = previous balance − repayment amount  (running)

    All loans are merged and sorted chronologically.
    Handles both POST (from selector/filter dropdown) and GET (?client_id=).
    """
    client_id = request.POST.get('client_id') or request.GET.get('client_id')
    ...
    # Branch from most recent loan
    branch_name = '—'
    latest_loan = LoanApplication.objects.filter(
        client=client
    ).order_by('-created_at').first()
    if latest_loan and latest_loan.office:
        branch_name = latest_loan.office.upper()
    ...
    # Each event: { sort_key, type, date, receipt_no, description,
    #               loan_type, loan_id_label, loan_amount, paid_amount,
    #               balance (filled in next pass) }
    events = []

    loans = LoanApplication.objects.filter(
        client=client
    ).prefetch_related('repayments').order_by('application_date', 'id')
    ...
    for loan in loans:
        ...
        events.append({
            'sort_key':     (loan.application_date, 0, loan.id),
            'type':         'disbursement',
            'date':         loan.application_date,
            ...
            'paid_amount':  Decimal('0.00'),
            '_loan_id':     loan.id,           # used for running balance per loan
        })

        # 2. Repayment entries (ordered by date then pk)
        for repayment in loan.repayments.all().order_by('repayment_date', 'id'):
            events.append({
                'sort_key':     (repayment.repayment_date, 1, repayment.id),
                'type':         'repayment',
                'date':         repayment.repayment_date,
                'receipt_no':   str(repayment.id).zfill(6),
                'description':  'Loan payment',
                'loan_type':    loan.loan_type,
                'loan_id_label': loan_id_label,
    ...
    # ── Compute running balance PER LOAN ─────────────────────────────
    # Each loan has its own running balance; we track them in a dict.
    loan_balances = {}
    ...
    for event in events:
        ...
        if event['type'] == 'disbursement':
            # Initialise or reset this loan's running balance
            loan_balances[loan_id] = event['loan_amount']
        else:
            # Deduct repayment from this loan's running balance
            loan_balances[loan_id] = loan_balances.get(loan_id, Decimal('0.00')) - event['paid_amount']
            loan_balances[loan_id] = max(loan_balances[loan_id], Decimal('0.00'))
        ...
        statement_rows.append({
            ...
            'loan_id_label': event['loan_id_label'],
            'loan_amount':  event['loan_amount'],
            'paid_amount':  event['paid_amount'],
            'balance':      loan_balances[loan_id],
        })
...
def branch_transaction_statement(request):
    ...
    Submits to branch_transaction_statement_report (POST).
    """
...
def branch_transaction_statement_report(request):
    """
    Receives the date range via POST, queries all branch transactions,
    and renders the full statement report on a separate page.

    Branch is auto-determined from request.user.office_allocation.

    Transaction types included:
      CREDIT:
        - LoanRepayment              → "Loan payment"
        - LoanTopup (old balance)    → "Clearance loan balance for top-up"

      DEBIT:
        - LoanApplication (new loan) → "Loan amount deposited to customer"
        - LoanTopup (new amount)     → "Loan amount deposited to customer"
        - Expense                    → "Category [description]," (bold)
    ...
    processed_by = request.user
    user_office  = getattr(processed_by, 'office_allocation', None)
    branch_name  = user_office.name if user_office else '—'
    ...
    office_name = user_office.name if user_office else None
    raw = []

    # ── 1. Loan Repayments (CREDIT) ───────────────────────────────────
    repayments = LoanRepayment.objects.filter(
        repayment_date__range=(date_from, date_to),
        loan_application__office=office_name,
    ).select_related(
        'loan_application__client', 'processed_by'
    ).order_by('repayment_date', 'id')
    ...
    for r in repayments:
        ...
        raw.append({
            'sort_key':         (r.repayment_date, 0, r.id),
    ...
    for t in topups:
        ...
        # DEBIT: new top-up amount disbursed
        raw.append({
            'sort_key':         (t.topup_date, 2, t.id),
            'date':             t.topup_date,
            'receipt_no':       str(t.id).zfill(6),
            'name':             client_name,
    ...
    disbursements = LoanApplication.objects.filter(
        application_date__range=(date_from, date_to),
        office=office_name,
    ).select_related(
        'client', 'processed_by'
    ...
    for loan in disbursements:
        if loan.id in topup_loan_ids:
            continue  # already covered above as a top-up
        c = loan.client
        raw.append({
            'sort_key':         (loan.application_date, 3, loan.id),
            'date':             loan.application_date,
            'receipt_no':       str(loan.id).zfill(6),
            'name':             f"{c.firstname} {c.middlename or ''} {c.lastname}".strip(),
            'description':      'Loan amount deposited to customer',
    ...
    expenses = Expense.objects.filter(
        ...
        'recorded_by', 'transaction_type'
    ).order_by('transaction_date', 'id')

    for exp in expenses:
        cat  = exp.transaction_type.name if exp.transaction_type else 'Expense'
        desc = f"{cat} [{exp.description}],"
        raw.append({
            'sort_key':         (exp.transaction_date, 4, exp.id),
    ...
    for sal in salaries:
        raw.append({
            'sort_key':         (sal.payment_date, 5, sal.id),
    ...
    # ── Mark repeated dates (show date only on first row of each date) ─
    prev_date = None
    for entry in raw:
...
def loan_collection_statement(request):
    """Renders the date-range filter form only."""
    return render(request, 'app/loan_collection_filter.html')
...
def loan_collection_statement_report(request):
    """
    Loan Collection Statement — CREDIT side only.

    Includes:
      - LoanRepayment records      → "Loan payment"
      - LoanTopup (old balance)    → "Clearance loan balance for top-up"

    Each row shows:
      Date | S/N # | Receipt # | Name | Description |
      Rate (%) | Principal | Interest | Total | Collected By

    Principal and Interest are derived from the repayment amount
    using the loan's interest rate ratio:
        interest_part  = repayment * (total_interest / total_repayment)
        principal_part = repayment - interest_part

    Scoped to: logged-in user's branch (office_allocation).
    """
    if request.method != 'POST':
    ...
    processed_by = request.user
    user_office  = getattr(processed_by, 'office_allocation', None)
    branch_name  = user_office.name.upper() if user_office else '—'
    office_name  = user_office.name if user_office else None
    ...
    # ── 2. Loan Top-up clearances (old balance paid back) ─────────────
    topups = LoanTopup.objects.filter(
        topup_date__range=(date_from, date_to),
        loan_application__office=office_name,
    ).select_related(
    ...
    for t in topups:
        ...
        total_rep   = loan.total_repayment_amount or Decimal('0')
        loan_amt    = loan.loan_amount or Decimal('0')
        old_balance = loan.repayment_amount_remaining or Decimal('0')
...
def bank_transfer_expenses_report(request):
    """
    Bank Transfer Expenses — shows ONLY OfficeTransaction records
    where transaction_method='bank' and office_from = logged-in user's branch.

    These are branch-to-branch transfers made via bank.
    Description: "Amount to Branch [destination_office_name]"
    """
    if request.method != 'POST':
    ...
    # ── Inter-branch bank transfers ONLY ─────────────────────────────
...
def bank_cash_transfer_report(request):
    """
    Bank ↔ Cash Transfer report.
    Shows ONLY BankCashTransaction records where
    office_from = logged-in user's branch.
    Description: "Bank to Cash transfer" or "Cash to Bank transfer"
    """
    if request.method != 'POST':
    ...
    txns = BankCashTransaction.objects.filter(
        transaction_date__range=(date_from, date_to),
        office_from=user_office,
    ).select_related('office_from').order_by('transaction_date', 'id')
    ...
    for txn in txns:
        ...
        rows.append({
            'date':         txn.transaction_date,
            'receipt_no':   str(txn.id).zfill(6),
            'name':         _full_name(processed_by),
            'description':  desc,
            'amount':       txn.amount,
            'processed_by': _full_name(processed_by),
        })
...
def loan_issued_report(request):
    """
    Monthly Loan Issued Report.

    Queries LoanApplication records for the logged-in user's branch
    within the selected date range.

    Columns:
      S/N | Date | Name | Check No. | Mobile No. | Work Station |
      Loan ID | Rate Type | Starting Month |
      Loaned Amount (Principal) | Period | Interest Rate % |
      Interest Amount | Total Amount (P+I) | Monthly Installment

    Work Station = client.kaziyako (job station / employer department)
    Starting Month = first_repayment_date formatted as "Mar/2026"
    Loan ID = "{office}-{loan.id}"  (e.g. mafinga-1562)
    Rate Type = "Flat" (all loans use flat rate in this system)

    GRAND TOTAL sums: Loaned Amount, Interest Amount,
    ...
    user         = request.user
    user_office  = getattr(user, 'office_allocation', None)
    branch_name  = user_office.name.upper() if user_office else '—'
    office_name  = user_office.name if user_office else None
    ...
    loans = LoanApplication.objects.filter(
        application_date__range=(date_from, date_to),
        office=office_name,
    ).select_related('client', 'processed_by').order_by('application_date', 'id')
...
def loan_report(request):
    """
    Branch Loan Report — single page, no date filter.

    Shows ALL LoanApplication records for the logged-in user's branch,
    sorted alphabetically by client last name then first name.

    One row per loan (a client with multiple loans appears multiple times).

    Columns:
      S/N | Name | Check No. | Mobile No. | Loan Type |
      Loan Amount | Interest Amount | Total | Paid Amount | Balance

    Paid Amount  = sum of all LoanRepayment.repayment_amount for this loan
    Balance      = loan.repayment_amount_remaining  (kept up-to-date by repayment view)
    Interest Amount = loan.total_interest_amount
    Total        = loan.total_repayment_amount  (Principal + Interest)

    GRAND TOTAL sums all numeric columns.
    """
    user        = request.user
    user_office = getattr(user, 'office_allocation', None)
    branch_name = user_office.name.upper() if user_office else '—'
...
def no_loan_customers(request):
    """
    Customers With No Existing Loan.

    Shows all Client records registered under the logged-in user's branch
    who have NO LoanApplication records at all — or whose most recent
    loan is fully paid (status='Paid' / repayment_amount_remaining=0).
    ...
    # Clients registered by staff in this office who have NO loans at all
    clients_no_loans = Client.objects.filter(
        registered_by__office_allocation=user_office,
        loan_applications__isnull=True,
    ).distinct().order_by('lastname', 'firstname')

    # Also include clients whose ALL loans are fully paid
    from django.db.models import Q
    clients_all_paid = Client.objects.filter(
        registered_by__office_allocation=user_office,
    ).exclude(
        loan_applications__repayment_amount_remaining__gt=0
    ).exclude(
        loan_applications__isnull=True  # already covered above
    ).distinct().order_by('lastname', 'firstname')
...
def expired_loans(request):
    """
    Expired Loans Report — single page, no date filter.

    A loan is considered EXPIRED when today's date is past its
    calculated END DATE (= first_repayment_date + payment_period_months).

    Only loans with repayment_amount_remaining > 0 are shown
    (fully paid loans are not expired, they're cleared).

    Columns:
      S/N | Name | Check no | Contact | Loan Type | Decision Date |
      Start Month | End Month | Loaned Amount | Paid Amount |
      Outstanding | Expired Days | Status

    Decision Date  = loan.application_date
    Start Month    = loan.first_repayment_date  (formatted Mon/YYYY)
    End Month      = first_repayment_date + payment_period_months - 1 month
                     (the last scheduled repayment month)
    Expired Days   = (today - end_date).days  — only positive values shown
    Paid Amount    = sum of all repayments for this loan
    Outstanding    = loan.repayment_amount_remaining

    Status:
      1–30 days   → Substandard
      31–90 days  → Doubtful
      91+ days    → Loss

    Sorted by expired_days ascending (least expired first).
    """
    user        = request.user
    user_office = getattr(user, 'office_allocation', None)
    branch_name = user_office.name.upper() if user_office else '—'
    ...
    today    = datetime.date.today()
    up_to_label = _month_label(today)   # e.g. "FEB/2026"

    loans = LoanApplication.objects.filter(
        office=office_name,
        repayment_amount_remaining__gt=0,   # not fully paid
    ).select_related('client').prefetch_related('repayments').order_by(
        'application_date', 'id'
    )
    ...
    for loan in loans:
        ...
        # End date = last repayment month (first_repayment_date + periods - 1 month)
        periods  = loan.payment_period_months
        end_date = loan.first_repayment_date + relativedelta(months=periods - 1)

        # Only include if end date has already passed
        if end_date >= today:
            continue
    ...
    # Sort by expired_days ascending (least overdue first, matching screenshot)
...
def monthly_outstanding_report(request):
    ...
    The user selects ANY date within a month (e.g. 02/03/2026).
    The report treats the ENTIRE selected month — regardless of the
    specific day chosen — as the period of interest.

    For every active loan in the branch (repayment_amount_remaining > 0),
    the algorithm:

    1.  Builds the full repayment schedule for the loan:
        - schedule[0]  → first_repayment_date  (month 1)
        - schedule[1]  → first_repayment_date + 1 month
        - ...
        - schedule[N-1]→ first_repayment_date + (N-1) months
        where N = payment_period_months.

    2.  Finds which schedule slot(s) fall in the selected month.
        A slot falls in the month when its year+month match the
        selected year+month.

    3.  "Amount to be paid" = sum of monthly_installment for those slots
        (normally 1 slot × monthly_installment, but handles edge cases).

    4.  "Paid amount (This month)" = sum of all LoanRepayment records
        whose repayment_date falls anywhere within the selected month
        (01/mm/yyyy – last day of mm/yyyy).  This captures ALL payments
        made during the month, not just on the exact day chosen.

    5.  "Not paid" = Amount to be paid − Paid amount (This month), floored at 0.

    6.  "Outstanding (Total)" = loan.repayment_amount_remaining
        (the live running balance of the entire loan).

    Loans whose schedule does not include the selected month at all
    (not yet started, or already fully finished) are EXCLUDED.
    ...
    # First and last day of the selected month (for payment lookup)
    import calendar
    last_day       = calendar.monthrange(sel_year, sel_month)[1]
    month_start    = datetime.date(sel_year, sel_month, 1)
    month_end      = datetime.date(sel_year, sel_month, last_day)
    month_label    = _month_label(selected_date).upper()   # e.g. "MAR/2026"
    ...
    for loan in loans:
        ...
        # Build schedule: list of (year, month) for each installment slot
        schedule_months = []
        for i in range(periods):
            slot_date = first_repay_date + relativedelta(months=i)
            schedule_months.append((slot_date.year, slot_date.month))

        # How many slots fall in the selected month?
        slots_in_month = sum(
            1 for (y, m) in schedule_months if y == sel_year and m == sel_month
        )

        # If this loan has no installment in the selected month → skip
        if slots_in_month == 0:
            continue
        ...
        # Payments actually made during the entire selected month
        paid_this_month = sum(
            r.repayment_amount
            for r in loan.repayments.all()
            if month_start <= r.repayment_date <= month_end
        )
...
def expenses_report(request):
    ...
    Shows all Expense records for the logged-in user's branch
    within the selected date range, ordered chronologically.

    Columns: Date | Receipt No. | Particular | Amount

    Particular = "Category [description]"
      e.g. "Staff rent [Kulipia Kodi (Meneja Kasubi)]"
      Category name is bold in the template.

    Receipt No:
      - Plain black for cash-paid expenses
      - Blue with 📄 icon for bank-paid expenses (is_bank=True)

    Date is shown only on the first row of each date group
    (repeated dates are hidden — matching the screenshot).

    GRAND TOTAL row has a light peach background with underlined amount.

    Expense model fields assumed:
      - transaction_date  (DateField)
      - transaction_type  (FK → ExpenseCategory with .name)
      - description       (TextField — the bracketed detail)
      - amount            (DecimalField)
      - payment_method    ('cash' or 'bank')
      - office            (CharField = office name, or FK)
      - recorded_by       (FK → User)
    """
    if request.method != 'POST':
...
def financial_statement_report(request):
    """
    Financial Statement — two-column Particular / Amount table.

    Row order (matches screenshot):
      Opening Balance                         ← calculated from history before start_date
      ─────────────
      MAPATO                                  ← loan repayments in period
      NYONGEZA                                ← Nyongeza model in period
      HAZINA                                  ← reserved / extend as needed
                            [income subtotal]
      ─────────────
      FOMU                                    ← loans disbursed in period (fixed)
      <CATEGORY NAME>                         ← cash expenses per ExpenseCategory (dynamic)
      MATUMIZI BENKI-[<CATEGORY NAME>]        ← bank expenses per ExpenseCategory (dynamic)
      MISHAHARA                               ← salaries in period (fixed)
                          [outflow subtotal]
      ─────────────
      BALANCE CASH                            ← live office_balance from BranchBalance
      BALANCE BENKI                           ← live bank_balance from BranchBalance
                           [balance subtotal]

    The expense section is fully DYNAMIC:
      - Loops all ExpenseCategory objects that have transactions in the period.
      - For each category:
          cash_amount  = sum of expenses with payment_method='cash'
          bank_amount  = sum of expenses with payment_method='bank'
      - Only categories with at least one non-zero sub-total appear.
      - Cash portion renders as:           "CATEGORY NAME"
      - Bank portion renders as:           "MATUMIZI BENKI-[CATEGORY NAME]"
      - If a category only has cash or only bank, only that row appears.
    """
    if request.method != 'POST':
    ...
    def rep_qs(q):
        ...
        if office_name:
            qs = qs.filter(loan_application__office=office_name)
        return qs

    def nyo_qs(q):
        qs = Nyongeza.objects.filter(q)
        if user_office:
            qs = qs.filter(Office=user_office)
        return qs

    def exp_qs(q):
        qs = Expense.objects.filter(q)
        if office_name:
            qs = qs.filter(office=office_name)
        return qs

    def loan_qs(q):
        qs = LoanApplication.objects.filter(q, status='Approved')
        if office_name:
            qs = qs.filter(office=office_name)
        return qs

    def sal_qs(q):
        qs = Salary.objects.filter(q)
        if user_office:
            qs = qs.filter(fund_source=user_office)
        return qs

    # ── Opening Balance (all activity BEFORE start_date) ───────────
    rep_b  = rep_qs(Q(repayment_date__lt=start_date)).aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')
    nyo_b  = nyo_qs(Q(date__lt=start_date)).aggregate(t=Sum('amount'))['t'] or Decimal('0')
    exp_b  = exp_qs(Q(expense_date__lt=start_date)).aggregate(t=Sum('amount'))['t'] or Decimal('0')
    loan_b = loan_qs(Q(application_date__lt=start_date)).aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')
    sal_b  = sal_qs(Q(salary_for_month__lt=start_date)).aggregate(t=Sum('amount'))['t'] or Decimal('0')
    opening_stock = (rep_b + nyo_b) - (exp_b + loan_b + sal_b)
    ...
    pq_loan = Q(application_date__gte=start_date,  application_date__lte=end_date)
    pq_sal  = Q(salary_for_month__gte=start_date,  salary_for_month__lte=end_date)

    # ── Income ─────────────────────────────────────────────────────
    total_mapato   = rep_qs(pq_rep).aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')
    total_nyongeza = nyo_qs(pq_nyo).aggregate(t=Sum('amount'))['t'] or Decimal('0')
    total_hazina   = Decimal('0')   # extend if you have a Hazina model
    total_income   = total_mapato + total_nyongeza + total_hazina
    total_income_with_opening = opening_stock + total_income

    # ── Expenditure: fixed items ────────────────────────────────────
    total_loans_current    = loan_qs(pq_loan).aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')
    total_salaries_current = sal_qs(pq_sal).aggregate(t=Sum('amount'))['t'] or Decimal('0')

    # ── Expenditure: dynamic expense categories ─────────────────────
    # Build one entry per ExpenseCategory that has activity in the period.
    # Each entry contains cash_amount and bank_amount separately so the
    # template can render them as separate rows.
    period_expenses = exp_qs(pq_exp).select_related('transaction_type')
    ...
    for cat in categories:
        ...
        cash_amount = cat_exps.filter(
            payment_method='cash'
        ).aggregate(t=Sum('amount'))['t'] or Decimal('0')

        bank_amount = cat_exps.filter(
            payment_method='bank'
        ).aggregate(t=Sum('amount'))['t'] or Decimal('0')

        # Only include categories with actual spend
    ...
    # Also catch expenses with no category (transaction_type=None)
    ...
    if unc_cash > 0 or unc_bank > 0:
        expenses_by_category.append({
            'name':        'Matumizi Mengineyo',
            'cash_amount': unc_cash if unc_cash > 0 else None,
            'bank_amount': unc_bank if unc_bank > 0 else None,
            'total':       unc_cash + unc_bank,
        })

    total_dynamic_expenses = sum(e['total'] for e in expenses_by_category)
    total_outflow_current  = total_loans_current + total_dynamic_expenses + total_salaries_current
    ...
    return render(request, 'app/financial_statement_report.html', {
        ...
        # Expenditure
        'total_loans_current':        total_loans_current    if total_loans_current    else None,
        'expenses_by_category':       expenses_by_category,   # ← DYNAMIC list
        'total_salaries_current':     total_salaries_current if total_salaries_current else None,
        'total_outflow_current':      total_outflow_current,
...
def monthly_repayment_report(request):
    """
    Monthly-Wise Loan Repayment Report.

    The user picks a Month From and Month To (MM-YYYY format).
    The report renders ONE table per month in that range.
    Each month table lists:
      - LoanRepayment records whose repayment_date falls in that calendar month
      - LoanTopup records (old-balance credit) whose topup_date falls in that month
    Both are CREDIT transactions for the branch.

    Columns:
      S/N | Transaction Date | Receipt No | Name | Check No. |
      Loan ID | Description | Amount

    Description:
      LoanRepayment → "Loan payment"
      LoanTopup     → "Clearance loan balance for top-up"

    Loan ID → "{office_name}-{loan.id}"  e.g. "mafinga-1155"

    Each month section ends with a GRAND TOTAL row (tan/gold).
    Months with no transactions still appear with an empty table
    and GRAND TOTAL: 0.

    Sections are separated by a gold/amber divider line.
    """
    if request.method != 'POST':
    ...
    # Build ordered list of (year, month) tuples in range
    period_months = []
    ...
    for (year, month) in period_months:
        ...
        label       = f"{MONTH_NAMES[month]}-{year}"   # e.g. "FEB-2026"
        ...
        for rep in repayments:
            ...
            name   = f"{client.firstname} {client.middlename or ''} {client.lastname}".strip()
            raw.append({
                'sort_key':      (rep.repayment_date, 0, rep.id),
        ...
        # ── LoanTopup records (old-balance credit) ────────────────
        topups = LoanTopup.objects.filter(
            topup_date__range=(month_start, month_end),
        ).select_related(
            'loan_application', 'loan_application__client', 'processed_by'
        ).order_by('topup_date', 'id')
        ...
        for topup in topups:
            ...
            client = loan.client
            name   = f"{client.firstname} {client.middlename or ''} {client.lastname}".strip()
            # The "old balance" cleared during top-up
            cleared = topup.old_balance_cleared or topup.topup_amount or Decimal('0')
            raw.append({
                'sort_key':      (topup.topup_date, 1, topup.id),
        ...
        # Sort all rows by date then type then id
...
def bank_transfer_expenses_result(request):
    """
    Page 2: Summary of bank transfer expenses per branch within date range.

    Shows ALL offices — even those with zero bank expenses in the period.

    Expense model fields used:
        expense_date      – DateField (auto_now_add → date of recording)
        transaction_date  – DateField (actual transaction date — preferred)
        payment_method    – CharField  → filter to 'bank'
        office            – CharField  (branch name as text)
        amount            – DecimalField
    """
    date_from_str = request.GET.get('date_from', '')
    ...
    # ── Get all offices sorted alphabetically ─────────────────────────────────
    all_offices = Office.objects.all().order_by('name')

    # ── Bank expenses in date range, grouped by office (CharField) ─────────────
    # Use transaction_date if set, otherwise fall back to expense_date
    bank_expenses = (
        Expense.objects.filter(
    ...
    # Build lookup dict: office_name → total
    expense_map = {row['office']: row['total'] for row in bank_expenses}

    # ── Merge: every office gets a row, zero if no expenses ───────────────────
    branch_summary = []
    for office in all_offices:
        branch_summary.append({
            'branch_name':  office.name,
            'total_amount': expense_map.get(office.name, None),  # None → blank cell like screenshot
        })

    # Also include any expenses recorded under office names not in Office table
    for office_name, total in expense_map.items():
        if office_name and not any(r['branch_name'] == office_name for r in branch_summary):
            branch_summary.append({
    ...
    branch_summary.sort(key=lambda x: x['branch_name'] or '')
...
def general_financial_statement_filter(request):
    ...
    """
    return render(request, 'app/general_financial_statement_filter.html')


# ══════════════════════════════════════════════════════════════════════
#  PAGE 2 — HQ Report (ALL offices combined)
# ══════════════════════════════════════════════════════════════════════


def general_financial_statement_report(request):
    """
    Financial Statement — ALL offices combined (HQ view).

    Structure (matching screenshot):
    ┌──────────────────────────────────────────────────────┐
    │  Opening Balance   (all activity before start_date)  │
    ├── INCOME ───────────────────────────────────────────┤
    │  Posted Amount     (loan repayments in period)       │
    │  Hazina            (Nyongeza model in period)        │
    │  External Sources  (OfficeTransaction received HQ)   │
    │                              [income_subtotal]        │
    ├── OUTFLOW ──────────────────────────────────────────┤
    │  Money to Branch   (OfficeTransaction sent from HQ)  │
    │  Loans Issued      (LoanApplication in period)       │
    │  <CATEGORY>        (cash expenses per category)      │
    │  MATUMIZI BENKI-X  (bank expenses per category)      │
    │  Mishahara         (Salary in period)                │
    │                              [outflow_subtotal]       │
    ├──────────────────────────────────────────────────────┤
    │  Closing Balance   = opening + income - outflow      │
    ├── LIVE BALANCES ────────────────────────────────────┤
    │  Balance Cash      (sum of all BranchBalance.office) │
    │  Balance Bank      (sum of all BranchBalance.bank)   │
    └──────────────────────────────────────────────────────┘
    """
    if request.method != 'POST':
        return redirect('financial_statement_filter')
    ...
    open_repayments = _sum(
        LoanRepayment.objects.filter(repayment_date__lt=start_date),
        'repayment_amount'
    )
    open_nyongeza = _sum(
        Nyongeza.objects.filter(date__lt=start_date),
    ...
    open_external = _sum(
        OfficeTransaction.objects.filter(transaction_date__lt=start_date),
        'amount'
    )

    # Outflow before period
    open_loans = _sum(
        LoanApplication.objects.filter(application_date__lt=start_date),
        'loan_amount'
    )
    open_expenses = _sum(
        Expense.objects.filter(expense_date__lt=start_date),
        'amount'
    )
    open_salaries = _sum(
        Salary.objects.filter(salary_for_month__lt=start_date),
    ...
    pq_loan = Q(application_date__gte=start_date,  application_date__lte=end_date)
    pq_sal  = Q(salary_for_month__gte=start_date,  salary_for_month__lte=end_date)
    pq_trx  = Q(transaction_date__gte=start_date,  transaction_date__lte=end_date)

    # ══ INCOME ════════════════════════════════════════════════════════

    # Posted Amount — all loan repayments across ALL offices
    total_repayments = _sum(
        LoanRepayment.objects.filter(pq_rep),
        'repayment_amount'
    )

    # Hazina — Nyongeza across ALL offices
    total_nyongeza = _sum(
        Nyongeza.objects.filter(pq_nyo),
        'amount'
    )

    # External Sources — OfficeTransactions received (all directions)
    total_external = _sum(
        OfficeTransaction.objects.filter(pq_trx),
        'amount'
    )
    ...
    # Money to Branch (OfficeTransactions — captured in external above,
    # kept as separate display row using same data for transparency)
    total_money_to_branch = total_external   # same OfficeTransaction data

    # Loans disbursed
    total_loans = _sum(
        LoanApplication.objects.filter(pq_loan),
        'loan_amount'
    )

    # Salaries
    total_salaries = _sum(
        Salary.objects.filter(pq_sal),
        'amount'
    )

    # ── Dynamic expense categories ─────────────────────────────────────
    period_expenses = Expense.objects.filter(pq_exp).select_related('transaction_type')
    categories = ExpenseCategory.objects.all().order_by('name')
    expenses_by_category = []
    ...
    unc_cash = unc.filter(payment_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')
    unc_bank = unc.filter(payment_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
    if unc_cash > 0 or unc_bank > 0:
        expenses_by_category.append({
            'name':        'Matumizi Mengineyo',
            'cash_amount': unc_cash if unc_cash > 0 else None,
    ...
    # Total outflow
    outflow_subtotal = total_loans + total_dynamic_expenses + total_salaries
    # Note: money_to_branch not double-counted in subtotal
    # (it's the same as external sources, shown for transparency)

    # ══ CLOSING BALANCE ═══════════════════════════════════════════════
    closing_balance = income_subtotal - outflow_subtotal

    # ══ LIVE BALANCES (from BranchBalance — latest per branch) ════════
    # Get the most recent BranchBalance per branch using subquery approach
    from django.db.models import Max

    # For each branch, get the latest record
    latest_ids = (
        BranchBalance.objects
        .values('branch')
        .annotate(latest=Max('last_updated'))
        .values_list('id', flat=False)
    )

    # Simpler approach: sum latest balance across all branches
    all_balances = BranchBalance.objects.order_by('branch', '-last_updated')
    seen_branches = set()
    total_cash_balance = Decimal('0')
    total_bank_balance = Decimal('0')

    for bb in all_balances:
        if bb.branch_id not in seen_branches:
            seen_branches.add(bb.branch_id)
            total_cash_balance += bb.office_balance
            total_bank_balance += bb.bank_balance

    total_live_balance = total_cash_balance + total_bank_balance
...
def branch_financial_summary(request):
    ...
    Rows (matches screenshot Image 2):
        Opening Balance          income_before - outflow_before
        MAPATO                   LoanRepayment in period
        NYONGEZA                 Nyongeza in period
        HAZINA (subtotal)        Opening + MAPATO + NYONGEZA
        FOMU                     LoanApplication (Approved) in period
        MATUMIZI OFISINI         Expense category 'Office Expenses'
        MATUMIZI-[KITUO]         Expense category 'Station Expenses'
        MATUMIZI-[MIKURUGENZI]   Expense category 'Director Expenses'
        MAKATO BENKI             Expense category 'Bank Charges'
        (outflow subtotal)       FOMU + all MATUMIZI rows
        BALANCE OFISINI          BranchBalance.office_balance (latest)
        BALANCE BENKI            BranchBalance.bank_balance   (latest)
        Closing Balance          HAZINA - outflow_subtotal
    """
    ...
    if not start_date_str or not end_date_str:
        # Redirect to filter with friendly message
        messages.error(request, 'Please select both dates.')
    ...
    totals = {
        ...
        'mapato':                  Decimal('0.00'),
        'nyongeza':                Decimal('0.00'),
        'hazina':                  Decimal('0.00'),
        'fomu':                    Decimal('0.00'),
        'matumizi':                Decimal('0.00'),
        'ofisini':                 Decimal('0.00'),
        'matumizi_kituo':          Decimal('0.00'),
        'matumizi_mikurugenzi':    Decimal('0.00'),
        'makato_benki':            Decimal('0.00'),
        'outflow_subtotal':        Decimal('0.00'),
        'balance_ofisini':         Decimal('0.00'),
        'balance_benki':           Decimal('0.00'),
    ...
    for office in offices:
        ...
        nyongeza_b = Nyongeza.objects.filter(
            Office=office, date__lt=start_date
        ).aggregate(t=Sum('amount'))['t'] or Decimal('0.00')

        repayments_b = LoanRepayment.objects.filter(
            loan_application__office=office,
            repayment_date__lt=start_date
        ).aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0.00')

        expenses_b = Expense.objects.filter(
            office=office,
            expense_date__lt=start_date
        ).aggregate(t=Sum('amount'))['t'] or Decimal('0.00')

        loans_b = LoanApplication.objects.filter(
            office=office,
            application_date__lt=start_date,
            status='Approved'
        ).aggregate(t=Sum('loan_amount'))['t'] or Decimal('0.00')

        salaries_b = Salary.objects.filter(
            fund_source=office,
            salary_for_month__lt=start_date
        ).aggregate(t=Sum('amount'))['t'] or Decimal('0.00')

        opening_balance = (nyongeza_b + repayments_b) - (expenses_b + loans_b + salaries_b)
        summary_data['opening_balances'][office.id] = opening_balance
        ...
        # ── HAZINA (income subtotal) ───────────────────────────────────────
        hazina = opening_balance + mapato + nyongeza
        summary_data['hazina'][office.id] = hazina
        totals['hazina'] += hazina

        # ── FOMU (loans disbursed) ────────────────────────────────────────
        ...
        # Base expense queryset for this office in period
        ...
        SPECIAL = ['Bank Charges', 'Office Expenses', 'Station Expenses', 'Director Expenses']

        # MATUMIZI (regular — excluding 4 special categories)
        matumizi = exp_base.exclude(
            transaction_type__name__in=SPECIAL
        ).aggregate(t=Sum('amount'))['t'] or Decimal('0.00')
        summary_data['matumizi'][office.id] = matumizi
        totals['matumizi'] += matumizi

        # MATUMIZI OFISINI
        ofisini = exp_base.filter(
            transaction_type__name='Office Expenses'
        ).aggregate(t=Sum('amount'))['t'] or Decimal('0.00')
        summary_data['ofisini'][office.id] = ofisini
        totals['ofisini'] += ofisini

        # MATUMIZI-KITUO
        kituo = exp_base.filter(
            transaction_type__name='Station Expenses'
        ).aggregate(t=Sum('amount'))['t'] or Decimal('0.00')
        summary_data['matumizi_kituo'][office.id] = kituo
        totals['matumizi_kituo'] += kituo

        # MATUMIZI-MIKURUGENZI
        mikurugenzi = exp_base.filter(
            transaction_type__name='Director Expenses'
        ).aggregate(t=Sum('amount'))['t'] or Decimal('0.00')
        summary_data['matumizi_mikurugenzi'][office.id] = mikurugenzi
        totals['matumizi_mikurugenzi'] += mikurugenzi

        # MAKATO BENKI
        makato_benki = exp_base.filter(
            transaction_type__name='Bank Charges'
        ).aggregate(t=Sum('amount'))['t'] or Decimal('0.00')
        ...
        bal_office = latest.office_balance if latest else Decimal('0.00')
        bal_bank   = latest.bank_balance   if latest else Decimal('0.00')
        summary_data['balance_ofisini'][office.id] = bal_office
        summary_data['balance_benki'][office.id]   = bal_bank
        totals['balance_ofisini'] += bal_office
        totals['balance_benki']   += bal_bank

        # ── CLOSING BALANCE ───────────────────────────────────────────────
        total_outflow = fomu + matumizi + ofisini + kituo + mikurugenzi + makato_benki
        closing = hazina - total_outflow
        summary_data['closing_balance'][office.id] = closing
        totals['closing_balance'] += closing
        summary_data['outflow_subtotal'][office.id] = total_outflow
        totals['outflow_subtotal'] += total_outflow
...
def salary_slip_filter(request):
    ...
    return render(request, 'app/salary_slip_filter.html')
...
def payroll_filter(request):
    return render(request, 'app/payroll_filter.html')
...
def payroll_submit(request):
    ...
    After processing, redirects back to the payroll report (GET) so the
    updated paid/unpaid state is shown with correct checked checkboxes.
    """
    if request.method != 'POST':
        return redirect('payroll_filter')
    ...
    for uid in pay_ids:
        ...
        Salary.objects.create(
            ...
            transaction_method = 'bank',
            fund_source        = user.office_allocation,
            processed_by       = request.user,
        )
        paid_count += 1
    ...
    # Redirect to GET version of payroll_report so the page refreshes cleanly
    return redirect(reverse('payroll_report') + f'?pm={payroll_month_str}')
...
def loans_owed_summary(request):
    user_office = getattr(request.user, 'office_allocation', None)
    office_name = user_office.name if user_office else None
...
def loans_owed_report(request):
    """
    SECTION A — Per-start-month tables
    ───────────────────────────────────
    Each active loan is placed in the table whose title matches its
    first_repayment_date month (e.g. MAR-2026).
    Only loans whose start month falls within the last 12 months are shown.

    For each loan the column range spans from the loan's start month
    through (start_month + payment_period_months - 1), i.e. the full
    repayment schedule.  Columns are capped to 12 months.

    Cell types:
      'tick'       — fully paid (paid >= installment)  → green bg, ✓
      'partial'    — paid > 0 but < installment        → green paid + rose out
      'out_only'   — past month, no payment            → rose out only
      'future_out' — current/future month, no payment  → plain out amount
      'empty'      — outside loan schedule             → blank

    SECTION B — HAMA tables
    ────────────────────────
    Any loan that has at least one past scheduled slot with payment < installment.
    Grouped by the YEAR of first_repayment_date (HAMA-2025, HAMA-2024, etc.).
    Column window = same rolling 12 months (win_start → today).
    Has extra 'Interest' column.
    """
    user_office = getattr(request.user, 'office_allocation', None)
    branch_name = user_office.name.upper() if user_office else '—'
    office_name = user_office.name if user_office else None
    ...
    # Rolling 12-month window (for HAMA columns and filtering)
    win_start  = today - relativedelta(months=11)
    ...
    # ── Build repayment map: {loan_id: {(y,m): total_paid}} ────────
    rep_map = {l.id: {} for l in loans_list}
    repayments = LoanRepayment.objects.filter(
        loan_application__in=loans_list
    ).values('loan_application_id', 'repayment_date', 'repayment_amount')
    for r in repayments:
        key = (r['repayment_date'].year, r['repayment_date'].month)
        lid = r['loan_application_id']
        rep_map[lid][key] = rep_map[lid].get(key, Decimal('0')) + r['repayment_amount']

    # ── Group loans by start month (only within last 12 months) ────
    groups = {}   # (start_y, start_m) -> [loan, ...]
    hama_candidate_ids = set()
    ...
    for loan in loans_list:
        ...
        # Only show groups within the 12-month window
        if (sy, sm) < (win_y, win_m) or (sy, sm) > (cur_y, cur_m):
            continue
    ...
    for (sy, sm) in sorted(groups.keys()):
        loans_in_group = sorted(
            groups[(sy, sm)],
            key=lambda l: (l.client.lastname or '', l.client.firstname or '')
        )
        # Column range: start month → start + period - 1, max 12 cols
        max_period = max((l.payment_period_months or 1) for l in loans_in_group)
        end_col    = (datetime.date(sy, sm, 1) + relativedelta(months=max_period - 1))
        col_months = _months_range(sy, sm, end_col.year, end_col.month)
        if len(col_months) > 12:
            col_months = col_months[:12]
        n_cols        = len(col_months)
        ...
        for loan in loans_in_group:
            ...
            for idx, (cy, cm) in enumerate(col_months):
                ...
                past        = (cy, cm) < (cur_y, cur_m)  # strictly before current month

                if inst > 0 and paid_this >= inst:
                    # Fully paid
                    cells.append({'type': 'tick'})
                    col_totals[idx]['paid'] += paid_this
                elif paid_this > 0:
                    # Partial
                    shortfall = max(inst - paid_this, Decimal('0'))
                    cells.append({'type': 'partial', 'paid': paid_this, 'out': shortfall})
                    col_totals[idx]['paid'] += paid_this
                    col_totals[idx]['out']  += shortfall
                    if past:
                        is_hama = True
                elif past and inst > 0:
                    # Past, no payment
                    cells.append({'type': 'out_only', 'out': inst})
                    col_totals[idx]['out'] += inst
                    is_hama = True
                elif inst > 0:
                    # Current or future slot
                    cells.append({'type': 'future_out', 'out': inst})
                    col_totals[idx]['out'] += inst
                else:
                    cells.append({'type': 'empty'})

            if is_hama:
                hama_candidate_ids.add(loan.id)
    ...
    # ── Build Section B: HAMA ───────────────────────────────────────
    # HAMA columns = rolling 12-month window
    hama_col_months  = _months_range(win_y, win_m, cur_y, cur_m)
    hama_month_hdrs  = [_label(y, m) for y, m in hama_col_months]

    hama_loans = [l for l in loans_list if l.id in hama_candidate_ids]

    # Group by year of first_repayment_date (descending)
    hama_year_groups = {}
    for loan in hama_loans:
        yr = loan.first_repayment_date.year if loan.first_repayment_date else 0
    ...
    for yr in sorted(hama_year_groups.keys(), reverse=True):
        ...
        for loan in h_loans:
            ...
            for idx, (cy, cm) in enumerate(hama_col_months):
                if not frd:
                    cells.append({'type': 'empty'})
                    continue
                ...
                if inst > 0 and paid_this >= inst:
                    cells.append({'type': 'tick'})
                    h_col_totals[idx]['paid'] += paid_this
                elif paid_this > 0:
                    shortfall = max(inst - paid_this, Decimal('0'))
                    cells.append({'type': 'partial', 'paid': paid_this, 'out': shortfall})
                    h_col_totals[idx]['paid'] += paid_this
                    h_col_totals[idx]['out']  += shortfall
                elif past and inst > 0:
                    cells.append({'type': 'out_only', 'out': inst})
                    h_col_totals[idx]['out'] += inst
                elif inst > 0:
                    cells.append({'type': 'future_out', 'out': inst})
                    h_col_totals[idx]['out'] += inst
                else:
...
def loans_owed_approve(request):
    if request.method == 'POST':
        ...
        if loan_ids:
            # Extend with your own approval logic here
            messages.success(request, f'Approved {len(loan_ids)} loan(s).')
        else:
