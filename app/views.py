...
from app.utils import get_office_balances
...
def clients(request):
    ...
    clients_qs = Client.objects.all().order_by('firstname')
    if filter_office:
        clients_qs = clients_qs.filter(registered_office=filter_office)
    ...
    if request.method == 'POST':
        ...
        try:
            with transaction.atomic():
                ...
                first_repayment_date_str = request.POST.get('application_date', '')
                for fmt in ('%d/%m/%Y', '%Y-%m-%d'):
                    try:
                        first_repayment_date = datetime.datetime.strptime(first_repayment_date_str, fmt).date()
                        break
                    except ValueError:
                        continue
                else:
                    first_repayment_date = datetime.date.today()
    
                processed_by          = request.user

                # topup_date_str = request.POST.get('topup_date', '')
                # topup_date = datetime.datetime.strptime(topup_date_str, '%Y-%m-%d').date()
                ...
                for fmt in ('%d/%m/%Y', '%Y-%m-%d'):
                    try:
                        topup_date = datetime.datetime.strptime(topup_date_str, fmt).date()
                        break
                    except ValueError:
                        continue
                else:
                    topup_date = datetime.date.today()
                ...
                # ── Real-time balance — no BranchBalance record needed ────
                cash_in_office, cash_in_bank = get_office_balances(office=branch_office)

                if transaction_method == 'cash':
                    if cash_in_office < net_disbursement:
                        messages.error(
                            request,
                            f'Insufficient cash balance. Available: TZS {cash_in_office:,.0f}/='
                        )
                        return redirect('process_loan_partb', pk=original_loan.client.id)
                else:
                    if cash_in_bank < net_disbursement:
                        messages.error(
                            request,
                            f'Insufficient bank balance. Available: TZS {cash_in_bank:,.0f}/='
                        )
                        return redirect('process_loan_partb', pk=original_loan.client.id)
        ...
        try:
            ...
            if payment_method == 'cash':
                if cash_in_office < amount:
                    messages.error(request, f'Insufficient cash balance. Available: TZS {cash_in_office:,.0f}/=')
                    return redirect('expense')
                if branch_balance:
                    BranchBalance.objects.create(
                        branch=branch_office,
                ...
                if cash_in_bank < amount:
                    messages.error(request, f'Insufficient bank balance. Available: TZS {cash_in_bank:,.0f}/=')
                    return redirect('expense')
                if branch_balance:
                    BranchBalance.objects.create(
                        branch=branch_office,
...
def loan_application(request):
    if request.method == 'POST':
        ...
        try:
            ...
            for fmt in ('%d/%m/%Y', '%Y-%m-%d'):
                try:
                    application_date = datetime.datetime.strptime(application_date_str, fmt).date()
                    break
                except ValueError:
                    continue
            ...
            if loan_type == 'Dharura':
                ...
                if active_dharura_count >= DHARURA_BRANCH_LIMIT:
                    messages.error(
                        request,
                        f'Branch "{branch_office.name}" has reached the maximum of '
            ...
            if transaction_method == 'cash':
                if cash_in_office < loan_amount_decimal:
                    messages.error(
                        request,
                        f'Insufficient cash balance. Available: TZS {cash_in_office:,.0f}/='
                    )
                ...
                if cash_in_bank < loan_amount_decimal:
                    messages.error(
                        request,
                        f'Insufficient bank balance. Available: TZS {cash_in_bank:,.0f}/='
                    )
...
def loan_repayment(request, loan_id):
    ...
    # ── Default repayment month — 19–18 cycle logic ───────────────────────
    # Day  1–18 → still in previous month's cycle  → show prev month
    # Day 19–31 → new cycle started                → show current month
    if today.day <= 18:
        default_repayment_month = (today - relativedelta(months=1)).strftime('%Y-%m')
    else:
        default_repayment_month = today.strftime('%Y-%m')
    ...
    if request.method == 'POST':
        ...
        repayment_date_str   = request.POST.get('transaction_date', '').strip()
        processed_by         = request.user
        ...
        try:
            ...
            # Cap at remaining balance — never overpay
            if repayment_amount > loan.repayment_amount_remaining:
                repayment_amount = loan.repayment_amount_remaining

            # Parse repayment_month: "YYYY-MM" → first day of that month
            payment_month = None
            if repayment_month_str:
                try:
                    payment_month = datetime.datetime.strptime(
                        repayment_month_str, '%Y-%m'
                    ).date()
                except ValueError:
                    payment_month = None

            # Parse repayment_date; fall back to today
            repayment_date = None
            if repayment_date_str:
                try:
                    repayment_date = datetime.datetime.strptime(
                        repayment_date_str, '%Y-%m-%d'
                    ).date()
                except ValueError:
                    pass
            ...
            # ── Update branch balance (snapshot pattern) ───────────────
            branch_balance = BranchBalance.objects.filter(branch=branch_office).last()
            is_cash        = (transaction_method == 'cash')

            if not branch_balance:
                BranchBalance.objects.create(
                    branch=branch_office,
                    office_balance=repayment_amount if is_cash else Decimal('0.00'),
                    bank_balance=repayment_amount   if not is_cash else Decimal('0.00'),
                    updated_by=processed_by,
                )
            else:
                BranchBalance.objects.create(
            ...
            # ── Mark loan as Paid if fully cleared ────────────────────
            loan.refresh_from_db()
            if loan.repayment_amount_remaining <= Decimal('0.00'):
                loan.status = 'Paid'
            ...
            messages.error(request, 'Invalid repayment amount. Please enter a valid number.')
        except Exception as e:
            messages.error(request, f'Error processing repayment: {str(e)}')
    ...
    context = {
        **base_ctx,
        ...
        'default_repayment_month': default_repayment_month,
    }
    return render(request, 'app/loan_repayment.html', context)
...
def bank_cash_transaction_add(request):
    ...
    Balances are computed dynamically via get_office_balances() —
    no BranchBalance record is required.
    """
    processed_by = request.user
    office_from  = get_selected_office(request)
    ...
    if request.method == 'POST':
        try:
            ...
            with transaction.atomic():
                # Real-time balances — no BranchBalance record needed
                cash_in_office, cash_in_bank = get_office_balances(office=office_from)

                if source == 'cash' and destination == 'bank':
                    if cash_in_office < amount:
                        messages.error(request, f'Insufficient cash. Available: TZS {cash_in_office:,.0f}')
                        return redirect('bank_cash_transaction')
    ...
    # GET — compute balances for display
    if office_from:
        cash_in_office, cash_in_bank = get_office_balances(office=office_from)
    else:
        cash_in_office, cash_in_bank = Decimal('0'), Decimal('0')
...
def branches_transaction_add(request):
    ...
    if request.method == 'POST':
        ...
        with transaction.atomic():
            ...
            if attachment:
                hq_txn.attachment = office_txn.attachment
                hq_txn.save(update_fields=['attachment'])
...
def loan_repayment_schedule(request, loan_id):
    ...
    # ── paid_by_month from LoanRepayment ─────────────────────────────────
    paid_by_month = {}
    for r in repayments:
        if r.payment_month:
    ...
    # ── Total topup amount (all topups combined) ──────────────────────────
    total_topup_paid = sum(topup_by_month.values(), Decimal('0.00'))
    ...
    # ── Pass 2: pool = repayments + ALL topups, apply top-down ───────────
    #
    # Topup inafunika outstanding balance yote — bila kujali payment_month.
    # Tunajumlisha topups ZOTE kwenye pool moja na ku-drain row kwa row.
    #
    regular_pool = sum(r['paid'] for r in scheduled_raw)
    remaining_pool = regular_pool + total_topup_paid
    ...
    for row in scheduled_raw:
        ...
        if remaining_pool >= t:
            remaining_pool -= t
            outstanding = Decimal('0.00')
        elif remaining_pool > Decimal('0.00'):
    ...
    final_outstanding = sum(
        (r['outstanding'] for r in schedule_rows),
        Decimal('0.00')
    )

    # ── Extra rows: repayments nje ya scheduled months ────────────────────
    ...
    # ── Extra rows: topups nje ya scheduled months ────────────────────────
    ...
    # ── Topup rows ndani ya scheduled months — onyesha kama extra display ─
    # (optional: kama unataka kuonyesha topup payments kwenye scheduled rows)
    for (year, month) in sorted(topup_by_month.keys()):
        if (year, month) in scheduled_months:
            paid = topup_by_month[(year, month)]
            schedule_rows.append({
                'month_label':  f"{month_names[month]}/{year} (Topup)",
                'principal':    Decimal('0.00'),
                'interest':     Decimal('0.00'),
                'penalty':      Decimal('0.00'),
                'total':        Decimal('0.00'),
    ...
    # ── Sort: scheduled kwanza, extras chini ─────────────────────────────
    scheduled_final = [r for r in schedule_rows if not r['extra']]
    extra_rows      = [r for r in schedule_rows if r['extra']]
    schedule_rows   = scheduled_final + extra_rows
...
def loan_collection_statement_report(request):
    ...
    dt_from = tz.localize(datetime.datetime.combine(date_from, datetime.time.min))
    dt_to   = tz.localize(datetime.datetime.combine(date_to,   datetime.time.max))

    # ── Helper: get borrower display name ────────────────────────────
    def get_name(loan):
        try:
            if loan.client:
                c = loan.client
                return f"{c.firstname} {c.middlename or ''} {c.lastname}".strip()
        except Exception:
            pass
        try:
            if loan.loan_group:
                return f"[Group] {loan.loan_group}"
        except Exception:
            pass
        return "Unknown"
    ...
    if office_name:
        candidate_topups = candidate_topups.filter(
            loan_application__office__iexact=office_name
        )
    ...
    for t in candidate_topups:
        match = LoanRepayment.objects.filter(
            ...
            repayment_date=t.topup_date,
        ).values_list('id', flat=True).first()
        if match:
            topup_repayment_ids_to_exclude.add(match)

    # ── Always initialise raw ─────────────────────────────────────────
    raw = []
    ...
    repayments = LoanRepayment.objects.filter(
        ...
        id__in=topup_repayment_ids_to_exclude,
    ).select_related(
        'loan_application',
        'loan_application__client',
    ).order_by('created_at', 'id')
    ...
    for r in repayments:
        ...
        # get processed_by name safely without select_related
        try:
            collector = _collector_name(r.processed_by) if r.processed_by_id else '—'
        except Exception:
            collector = '—'
    ...
    for t in topups:
        ...
        try:
            collector = _collector_name(t.processed_by) if t.processed_by_id else '—'
        except Exception:
            collector = '—'
...
def expired_loans(request):
    ...
    loans = LoanApplication.objects.filter(
        ...
        'client__firstname', 'application_date', 'id'
    )
...
def monthly_outstanding_report(request):
    ...
    # Cutoff ya ripoti — haiwezi kuzidi 'today'.
    due_cutoff = min(month_end, today)

    # ── Deadline BASE ya installment ya KWANZA, kutoka application_date ────
    # Sheria:
    #   siku 1–18  ya mwezi wa maombi  → deadline = tarehe 18 MWEZI HUOHUO
    #   siku 19–31 ya mwezi wa maombi  → deadline = tarehe 18 MWEZI UNAOFUATA
    ...
    for loan in loans:
        ...
        # ── Ratiba ya DEADLINE (tarehe 18 kila mwezi, kuanzia deadline_base) ──
        deadline_base = _deadline_base(loan.application_date)
        schedule_deadlines = [
            deadline_base + relativedelta(months=i)
            for i in range(periods)
        ]
        # Overdue huanza siku INAYOFUATA ya deadline (19)
        overdue_starts = [d + datetime.timedelta(days=1) for d in schedule_deadlines]
        last_deadline  = schedule_deadlines[-1]

        # Kama hata installment ya kwanza bado haijafika 'overdue' — mkopo
        # huu haujaanza ku-due kwa ripoti hii kabisa.
        if not schedule_deadlines or overdue_starts[0] > due_cutoff:
            continue

        # ── Malipo yote — kwa ajili ya FIFO/arrears tumia payment_month
        # kwanza (ndio mwezi HALISI unaolipiwa), la sivyo repayment_date. ────
        def _ref_date(r):
        ...
        # ── FIFO allocation: shilingi ya kwanza inafunika installment ya
        # ZAMANI zaidi kabla ya kuendelea na inayofuata ──────────────────
        slot_remaining = [monthly_inst for _ in schedule_deadlines]
        pool = sum((r.repayment_amount or Decimal('0.00')) for r in all_repayments)
        ...
        # ── Installment zilizopita 'overdue_start' (<= due_cutoff) na
        # bado hazijalipwa kikamilifu ──────────────────────────────────
        unpaid_slots = [
            (schedule_deadlines[i], slot_remaining[i])
            for i in range(periods)
            if overdue_starts[i] <= due_cutoff and slot_remaining[i] > Decimal('0.00')
        ]

        # ────────────────────────────────────────────────────────────────
        # KIGEZO PEKEE cha kuonyesha: kuna installment(s) zilizopita
        # deadline (19+) na bado hazijalipwa kikamilifu?
        #   - Hakuna → HAAONEKANI (hata alilipa baadaye ya deadline,
        #     ilimradi kabla ya leo).
        #   - Zipo → ANAONEKANA, ataendelea kuonekana MPAKA azimalize.
        # ────────────────────────────────────────────────────────────────
        if not unpaid_slots:
            continue

        not_paid = sum(r for (_, r) in unpaid_slots)

        # ── "Amount to be paid" = JUMLA ya installment ZOTE zilizopita
        # deadline na bado hazijalipwa (ikiwa ni miezi 2+ zote zinajumlishwa)
        amount_to_be_paid = not_paid

        # ── Lebo ya miezi ipi hasa haijalipwa (uwazi kamili) ───────────────
        unpaid_periods_label = ', '.join(
            d.strftime('%b-%Y') for (d, _) in unpaid_slots
        )

        # Mkopo umeshapita muda wake wote wa mkataba (HAMA sugu)
        is_overdue_loan = last_deadline < month_start

        # ── Alicholipa KIHALISIA (cash iliyoingia) ndani ya mwezi wa
        # kalenda uliochaguliwa — kwa kutumia repayment_date (siyo
        # payment_month), kwa sababu hii ni "cash received this month"
        # siyo "deni la mwezi gani hasa linalipwa". ─────────────────────
        paid_this_month = sum(
        ...
        rows.append({
            ...
            'unpaid_periods_label':  unpaid_periods_label,
            'unpaid_periods_count':  len(unpaid_slots),
        })
...
def financial_statement_report(request):
    ...
    # ── Q filters ─────────────────────────────────────────────────────────────
    before_q = Q(created_at__lt=start_dt)
    pq       = Q(created_at__gte=start_dt, created_at__lte=end_dt)

    # ═════════════════════════════════════════════════════════════════════════
    # OPENING BALANCE (everything BEFORE start_date) split by cash / bank
    # ═════════════════════════════════════════════════════════════════════════

    # ── Opening cash IN ───────────────────────────────────────────────────────
    cash_rep_b = rep_qs(before_q).exclude(
        loan_application__loan_type__iexact='Hazina'
    ).filter(transaction_method__iexact='cash').aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')

    cash_hazina_b = rep_qs(before_q).filter(
        loan_application__loan_type__iexact='Hazina',
        transaction_method__iexact='cash',
    ).aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')

    cash_nyo_b = nyo_qs(before_q).filter(
        deposit_method__iexact='cash'
    ).aggregate(t=Sum('amount'))['t'] or Decimal('0')
    
    # ── Opening internal transfers ────────────────────────────────────────────
    cash_to_bank_b = bank_cash_txn_qs(before_q).filter(source__iexact='cash', destination__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
    bank_to_cash_b = bank_cash_txn_qs(before_q).filter(source__iexact='bank', destination__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')

    # ── Opening cash OUT ──────────────────────────────────────────────────────
    cash_exp_b  = exp_qs(before_q).filter(payment_method__iexact='cash').aggregate(t=Sum('amount'))['t']      or Decimal('0')
    cash_loan_b = loan_qs(before_q).filter(transaction_method__iexact='cash').aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')
    cash_charge_b = bank_charge_qs(before_q).filter(payment_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')

    opening_cash = (
        cash_rep_b + cash_hazina_b + cash_nyo_b + bank_to_cash_b
    ) - (
        cash_exp_b + cash_loan_b + cash_charge_b + cash_to_bank_b
    )

    # ── Opening bank IN ───────────────────────────────────────────────────────
    bank_rep_b = rep_qs(before_q).exclude(
        loan_application__loan_type__iexact='Hazina'
    ).filter(transaction_method__iexact='bank').aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')

    bank_hazina_b = rep_qs(before_q).filter(
        loan_application__loan_type__iexact='Hazina',
        transaction_method__iexact='bank',
    ).aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')

    bank_nyo_b         = nyo_qs(before_q).filter(deposit_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
    bank_transfer_in_b = transfer_in_qs(before_q).aggregate(t=Sum('amount'))['t'] or Decimal('0')

    # ── Opening bank OUT ──────────────────────────────────────────────────────
    bank_exp_b          = exp_qs(before_q).filter(payment_method__iexact='bank').aggregate(t=Sum('amount'))['t']       or Decimal('0')
    bank_loan_b         = loan_qs(before_q).filter(transaction_method__iexact='bank').aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')
    bank_transfer_out_b = transfer_out_qs(before_q).aggregate(t=Sum('amount'))['t'] or Decimal('0')
    bank_charge_b       = bank_charge_qs(before_q).filter(payment_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')

    opening_bank = (
        bank_rep_b + bank_hazina_b + bank_nyo_b + bank_transfer_in_b + cash_to_bank_b
    ) - (
        bank_exp_b + bank_loan_b + bank_transfer_out_b + bank_charge_b + bank_to_cash_b
    )

    opening_stock = opening_cash + opening_bank

    # ═════════════════════════════════════════════════════════════════════════
    # PERIOD INCOME (Inflows)
    # ═════════════════════════════════════════════════════════════════════════
    total_mapato = rep_qs(pq).exclude(
        loan_application__loan_type__iexact='Hazina'
    ).aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')

    total_nyongeza = nyo_qs(pq).aggregate(t=Sum('amount'))['t'] or Decimal('0')

    total_hazina = rep_qs(pq).filter(
        loan_application__loan_type__iexact='Hazina'
    ).aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')
    ...
    loan_cash_amount      = period_loans.filter(transaction_method__iexact='cash').aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')
    loan_bank_amount      = period_loans.filter(transaction_method__iexact='bank').aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')
    ...
    for cat in office_expense_categories:
        ...
        cash_amount = cat_exps.filter(payment_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')
        bank_amount = cat_exps.filter(payment_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
    ...
    unc_cash  = uncategorised.filter(payment_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')
    unc_bank  = uncategorised.filter(payment_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
    ...
    makato_benki_cash   = period_bank_charges.filter(payment_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')
    makato_benki_bank   = period_bank_charges.filter(payment_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
    ...
    # ═════════════════════════════════════════════════════════════════════════
    # CLOSING BALANCE (calculated from transactions, not BranchBalance snapshot)
    # ═════════════════════════════════════════════════════════════════════════

    # ── Period cash IN ────────────────────────────────────────────────────────
    cash_rep_period = rep_qs(pq).exclude(
        loan_application__loan_type__iexact='Hazina'
    ).filter(transaction_method__iexact='cash').aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')

    cash_hazina_period = rep_qs(pq).filter(
        loan_application__loan_type__iexact='Hazina',
        transaction_method__iexact='cash',
    ).aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')

    cash_nyo_period = nyo_qs(pq).filter(
        deposit_method__iexact='cash'
    ).aggregate(t=Sum('amount'))['t'] or Decimal('0')
    
    # ── Period internal transfers ─────────────────────────────────────────────
    cash_to_bank_period = bank_cash_txn_qs(pq).filter(source__iexact='cash', destination__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
    bank_to_cash_period = bank_cash_txn_qs(pq).filter(source__iexact='bank', destination__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')

    # ── Period cash OUT ───────────────────────────────────────────────────────
    cash_exp_period    = sum(r['cash_amount'] or Decimal('0') for r in matumizi_ofisini_rows)
    cash_charge_period = makato_benki_cash
    ...
    # ── Period bank IN ────────────────────────────────────────────────────────
    bank_rep_period = rep_qs(pq).exclude(
        loan_application__loan_type__iexact='Hazina'
    ).filter(transaction_method__iexact='bank').aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')

    bank_hazina_period = rep_qs(pq).filter(
        loan_application__loan_type__iexact='Hazina',
        transaction_method__iexact='bank',
    ).aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')

    bank_nyo_period         = nyo_qs(pq).filter(deposit_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
    bank_transfer_in_period = transfer_in_qs(pq).aggregate(t=Sum('amount'))['t'] or Decimal('0')

    # ── Period bank OUT ───────────────────────────────────────────────────────
    bank_exp_period          = sum(r['bank_amount'] or Decimal('0') for r in matumizi_ofisini_rows)
    bank_transfer_out_period = total_matumizi_kituo + total_matumizi_mkurugenzi
    bank_charge_period       = makato_benki_bank
    ...
    return render(request, 'app/financial_statement_report.html', {
        **base_ctx,
        ...
        # Expenditure 1: Matumizi Ofisini
        ...
        # Expenditure 2: Matumizi Benki-[Kituo]
        ...
        # Closing balances (calculated from transactions)
        'cash_in_office': cash_in_office,
        'cash_in_bank':   cash_in_bank,
        'total_cash':     total_cash,
    })
...
def monthly_repayment_report(request):
    ...
    repayment_date__range=(range_start, range_end),
        ...
        'loan_application', 'loan_application__client'  # removed processed_by
    ).order_by('payment_month', 'id')
    ...
    for rep in repayments:
        payment_month = rep.payment_month or rep.repayment_date
        if not payment_month:
            continue
        loan   = rep.loan_application
        client = loan.client
        key    = (payment_month.year, payment_month.month)
        buckets.setdefault(key, []).append({
            'sort_key':      (payment_month, 0, rep.id),
            'date':          rep.repayment_date,
            'receipt_no':    str(rep.id).zfill(6),
            'name':          f"{client.firstname} {client.middlename or ''} {client.lastname}".strip(),
            'check_no':      client.checkno or client.employmentcardno or '—',
    ...
    for topup in topups:
        payment_month = topup.payment_month or topup.topup_date
        if not payment_month:
            continue
        loan    = topup.loan_application
        client  = loan.client
        ...
        key     = (payment_month.year, payment_month.month)
        buckets.setdefault(key, []).append({
            'sort_key':      (payment_month, 1, topup.id),
            'date':          topup.topup_date,
            'receipt_no':    str(topup.id).zfill(6),
            'name':          f"{client.firstname} {client.middlename or ''} {client.lastname}".strip(),
            'check_no':      client.checkno or client.employmentcardno or '—',
    ...
    for key in sorted(buckets.keys()):
        year, month = key
        rows = sorted(buckets[key], key=lambda r: r['name'].upper())
        months_data.append({
            'label':       f"{MONTH_NAMES[month]}-{year}",
            'rows':        rows,
            'grand_total': sum(r['amount'] for r in rows),
...
def loans_issued_report_result(request):
    ...
    # ── Loan aggregates for offices that have loans in range ──────────
    loan_data = {}
    for row in (
        LoanApplication.objects
        .filter(
            ...
            client__isnull=False,        # match detail view — skip orphan loans
        )
        .values('office')
        .annotate(
        ...
        loan_data[row['office']] = row

    # ── All offices — excluding HQ, including zero-loan branches ─────
    all_offices = (
        Office.objects
        .exclude(name__iexact='HQ')
        .values_list('name', flat=True)
        .order_by('name')
    )
    ...
    for office_name in all_offices:
        ...
        branch_summary.append({
            ...
            'loaned_amount':   row['loaned_amount']   if row else Decimal('0'),
            'interest_amount': row['interest_amount'] if row else Decimal('0'),
            'total_return':    row['total_return']    if row else Decimal('0'),
        })

    # ── Orphan office names (not in Office table, also skip HQ) ──────
    known_office_names = set(all_offices)
    for office_name, row in loan_data.items():
        if office_name not in known_office_names and (office_name or '').upper() != 'HQ':
            branch_summary.append({
                'branch_name':     office_name or 'N/A',
    ...
    branch_summary.sort(key=lambda r: r['branch_name'])
    ...
    return render(request, 'app/loans_issued_report_result.html', {
        **get_base_context(request),
        'date_from':      date_from,
        'date_to':        date_to,
...
def general_financial_statement_report(request):
    ...
    # ════════════════════════════════════════════════════════════════
    # OPENING BALANCE (everything BEFORE start_date, ALL branches)
    # ════════════════════════════════════════════════════════════════

    # Repayments — repayment_date is DateField
    open_repayments = _sum(
        LoanRepayment.objects.filter(repayment_date__lt=start_date),
        'repayment_amount'
    )

    # Nyongeza — date is DateField
    open_nyongeza = _sum(
        Nyongeza.objects.filter(date__lt=start_date),
        'amount'
    )

    # Transfers in — transaction_date is DateField
    # Exclude internal branch-to-branch to avoid double counting
    open_external = _sum(
    ...
    # Loans disbursed — created_at is DateTimeField
    open_loans = _sum(
        LoanApplication.objects.filter(created_at__lt=before_dt),
        'loan_amount'
    )
    ...
    # Salaries — salary_for_month is DateField
    open_salaries = _sum(
        Salary.objects.filter(salary_for_month__lt=start_date),
        'amount'
    )

    # Bank charges — expense_date is DateField
    open_bank_charges = _sum(
        BankCharge.objects.filter(expense_date__lt=start_date),
        'amount'
    )
    ...
    # ════════════════════════════════════════════════════════════════
    # CURRENT PERIOD — ALL branches
    # ════════════════════════════════════════════════════════════════

    # ── INCOME ───────────────────────────────────────────────────────

    # Repayments — DateField
    ...
    # Only count HQ→Branch transfers as external income to avoid
    # double-counting branch-to-branch transfers
    # total_external = _sum(
    #     OfficeTransaction.objects.filter(
    ...
    # Branch-to-branch transfers excluded from outflow — they cancel out
    # Only HQ→Branch transfers counted as outflow at HQ level
    total_hq_transfers_out = _sum(
        OfficeTransaction.objects.filter(
            transaction_date__gte=start_date,
            transaction_date__lte=end_date,
            office_from__name__iexact='HQ',
    ...
    context = {
        **base_ctx,
        ...
        'total_hq_transfers_out': total_hq_transfers_out,
...
def branch_financial_summary(request):
    ...
    start_dt = timezone.make_aware(datetime.datetime.combine(start_date, datetime.time.min))
    end_dt   = timezone.make_aware(datetime.datetime.combine(end_date,   datetime.time.max))
    before_dt = start_dt  # everything strictly before start
    ...
    for office in offices:
        ...
        # ════════════════════════════════════════════════════════════
        # OPENING BALANCE (everything BEFORE start_date)
        # Uses DateField where available, DateTimeField with aware dt
        # ════════════════════════════════════════════════════════════
        ...
        # OfficeTransaction — transaction_date is DateField
        trx_in_b = OfficeTransaction.objects.filter(
            office_to=office,
            transaction_date__lt=start_date,
        ).aggregate(t=Sum('amount'))['t'] or Decimal('0')

        # Expense — expense_date is DateField
        exp_b = Expense.objects.filter(
            office=office.name,
            expense_date__lt=start_date,
        ).aggregate(t=Sum('amount'))['t'] or Decimal('0')

        # LoanApplication — created_at is DateTimeField, use aware dt
        ...
        # BankCharge — expense_date is DateField
        ...
        # MAPATO — repayment_date is DateField
        ...
        # HAZINA — repayment_date is DateField
        ...
        # INCOME SUBTOTAL
        income_subtotal = (
            opening_balance + mapato + nyongeza + transfers_in #+ hazina
        )
        ...
        # FOMU — created_at is DateTimeField, use aware range
        ...
        # MATUMIZI OFISINI — expense_date is DateField
        ...
        # MATUMIZI BENKI-[KITUO] — transaction_date is DateField
        ...
        # MATUMIZI BENKI-[MKURUGENZI] — transaction_date is DateField
        ...
        # MAKATO BENKI — expense_date is DateField
        ...
        # ════════════════════════════════════════════════════════════
        # CLOSING BALANCE (calculated, not from BranchBalance snapshot)
        # closing = income_subtotal - outflow_subtotal
        # Split into cash vs bank using transaction_method
        # ════════════════════════════════════════════════════════════

        # Opening cash
        cash_rep_b = LoanRepayment.objects.filter(
            loan_application__office=office.name,
            repayment_date__lt=start_date,
        ).exclude(
            loan_application__loan_type__iexact='Hazina'
        ...
        cash_hazina_b = LoanRepayment.objects.filter(
            loan_application__office=office.name,
            repayment_date__lt=start_date,
            loan_application__loan_type__iexact='Hazina',
            transaction_method__iexact='cash',
        ...
        cash_nyo_b   = Nyongeza.objects.filter(Office=office, date__lt=start_date, deposit_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')
        cash_exp_b   = Expense.objects.filter(office=office.name, expense_date__lt=start_date, payment_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')
        cash_loan_b  = LoanApplication.objects.filter(office=office.name, created_at__lt=before_dt, transaction_method__iexact='cash').aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')
        cash_chg_b   = BankCharge.objects.filter(office=office.name, expense_date__lt=start_date, payment_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')
        cash_to_bank_b = BankCashTransaction.objects.filter(office_from=office, transaction_date__lt=start_date, source__iexact='cash', destination__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
        bank_to_cash_b = BankCashTransaction.objects.filter(office_from=office, transaction_date__lt=start_date, source__iexact='bank', destination__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')

        opening_cash = (cash_rep_b + cash_hazina_b + cash_nyo_b + bank_to_cash_b) - (cash_exp_b + cash_loan_b + cash_chg_b + cash_to_bank_b)

        # Opening bank
        bank_rep_b    = LoanRepayment.objects.filter(loan_application__office=office.name, repayment_date__lt=start_date).exclude(loan_application__loan_type__iexact='Hazina').filter(transaction_method__iexact='bank').aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')
        bank_haz_b    = LoanRepayment.objects.filter(loan_application__office=office.name, repayment_date__lt=start_date, loan_application__loan_type__iexact='Hazina', transaction_method__iexact='bank').aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')
        bank_nyo_b    = Nyongeza.objects.filter(Office=office, date__lt=start_date, deposit_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
        bank_trxin_b  = OfficeTransaction.objects.filter(office_to=office, transaction_date__lt=start_date).aggregate(t=Sum('amount'))['t'] or Decimal('0')
        bank_exp_b    = Expense.objects.filter(office=office.name, expense_date__lt=start_date, payment_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
        bank_loan_b   = LoanApplication.objects.filter(office=office.name, created_at__lt=before_dt, transaction_method__iexact='bank').aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')
        bank_trxout_b = OfficeTransaction.objects.filter(office_from=office, transaction_date__lt=start_date).aggregate(t=Sum('amount'))['t'] or Decimal('0')
        bank_chg_b    = BankCharge.objects.filter(office=office.name, expense_date__lt=start_date, payment_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
        # opening_bank  = (bank_rep_b + bank_haz_b + bank_nyo_b + bank_trxin_b) - (bank_exp_b + bank_loan_b + bank_trxout_b + bank_chg_b)
        opening_bank = (
            bank_rep_b + bank_haz_b + bank_nyo_b + bank_trxin_b + cash_to_bank_b
        ) - (
            bank_exp_b + bank_loan_b + bank_trxout_b + bank_chg_b + bank_to_cash_b
        )

        # Period cash IN
        cash_rep_p   = LoanRepayment.objects.filter(loan_application__office=office.name, repayment_date__gte=start_date, repayment_date__lte=end_date).exclude(loan_application__loan_type__iexact='Hazina').filter(transaction_method__iexact='cash').aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')
        cash_haz_p   = LoanRepayment.objects.filter(loan_application__office=office.name, repayment_date__gte=start_date, repayment_date__lte=end_date, loan_application__loan_type__iexact='Hazina', transaction_method__iexact='cash').aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')
        cash_nyo_p   = Nyongeza.objects.filter(Office=office, date__gte=start_date, date__lte=end_date, deposit_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')
        cash_to_bank_p = BankCashTransaction.objects.filter(office_from=office, transaction_date__gte=start_date, transaction_date__lte=end_date, source__iexact='cash', destination__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
        bank_to_cash_p = BankCashTransaction.objects.filter(office_from=office, transaction_date__gte=start_date, transaction_date__lte=end_date, source__iexact='bank', destination__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')

        

        # Period cash OUT
        cash_loan_p  = LoanApplication.objects.filter(office=office.name, created_at__gte=start_dt, created_at__lte=end_dt, transaction_method__iexact='cash').aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')
        cash_exp_p   = Expense.objects.filter(office=office.name, expense_date__gte=start_date, expense_date__lte=end_date, payment_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')
        cash_chg_p   = BankCharge.objects.filter(office=office.name, expense_date__gte=start_date, expense_date__lte=end_date, payment_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')

        # bal_cash = opening_cash + cash_rep_p + cash_nyo_p - cash_loan_p - cash_exp_p - cash_chg_p #+ cash_haz_p
        bal_cash = (
            opening_cash
            + cash_rep_p + cash_nyo_p + bank_to_cash_p
            - cash_loan_p - cash_exp_p - cash_chg_p - cash_to_bank_p
        )

        # Period bank IN
        bank_rep_p    = LoanRepayment.objects.filter(loan_application__office=office.name, repayment_date__gte=start_date, repayment_date__lte=end_date).exclude(loan_application__loan_type__iexact='Hazina').filter(transaction_method__iexact='bank').aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')
        bank_haz_p    = LoanRepayment.objects.filter(loan_application__office=office.name, repayment_date__gte=start_date, repayment_date__lte=end_date, loan_application__loan_type__iexact='Hazina', transaction_method__iexact='bank').aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')
        bank_nyo_p    = Nyongeza.objects.filter(Office=office, date__gte=start_date, date__lte=end_date, deposit_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
        bank_trxin_p  = OfficeTransaction.objects.filter(office_to=office, transaction_date__gte=start_date, transaction_date__lte=end_date).aggregate(t=Sum('amount'))['t'] or Decimal('0')

        # Period bank OUT
        bank_loan_p    = LoanApplication.objects.filter(office=office.name, created_at__gte=start_dt, created_at__lte=end_dt, transaction_method__iexact='bank').aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')
        bank_exp_p     = Expense.objects.filter(office=office.name, expense_date__gte=start_date, expense_date__lte=end_date, payment_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')
        bank_trxout_p  = OfficeTransaction.objects.filter(office_from=office, transaction_date__gte=start_date, transaction_date__lte=end_date).aggregate(t=Sum('amount'))['t'] or Decimal('0')
        bank_chg_p     = BankCharge.objects.filter(office=office.name, expense_date__gte=start_date, expense_date__lte=end_date, payment_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')

        # bal_bank  = opening_bank + bank_rep_p + bank_nyo_p + bank_trxin_p - bank_loan_p - bank_exp_p - bank_trxout_p - bank_chg_p # + bank_haz_p
        bal_bank = (
            opening_bank
            + bank_rep_p + bank_nyo_p + bank_trxin_p + cash_to_bank_p
            - bank_loan_p - bank_exp_p - bank_trxout_p - bank_chg_p - bank_to_cash_p
        )
...
def _distribute_payments(col_months, rmap=None, inst=Decimal('0'), frd=None,
    ...
    • Topup month → onyesha rmap_this_month + topup_slots_covered_amount.
    • Topup month entry kwenye rmap INABAKI — topup_lump inaongezwa juu yake.
    ...
    # Hatuondoi topup_month kutoka clean_rmap — malipo ya kawaida inabaki.
    # Topup_lump inaongezwa juu kwenye all_payment_events tu.
    ...
    # Gawanya clean_rmap kwenye slots (malipo ya kawaida YOTE ikiwemo topup_month)
    ...
    # Gawanya topup_lump kwenye slots NA RECORD kiasi kilichofunikwa
    topup_slots_covered_amount = Decimal('0')
    ...
    if topup_month is not None and topup_lump > 0:
        ...
        for off in range(topup_offset):
            ...
            slot_covered[off] += take
            remaining         -= take
            topup_slots_covered_amount += take
    ...
    for (cy, cm) in col_months:
        ...
        if 0 <= offset < period:
            ...
            # paid_display:
            # topup_month → rmap_this_month (248k) + topup_slots_covered_amount (742k) = 990k
            # kawaida     → rmap_this_month tu
            if topup_month is not None and (cy, cm) == topup_month:
                paid_display = rmap_this_month + topup_slots_covered_amount
...
def loans_owed_report(request):
    ...
    for (sy, sm) in sorted(groups.keys(), reverse=True):
        loans_in_group = sorted(
            groups[(sy, sm)],
            key=lambda l: (l.client.firstname or '', l.client.lastname or '')
        )
...
def bank_charges_filter(request):
    return render(request, 'app/bank_charges_filter.html', {
        **get_base_context(request),
    })


def bank_charges_report(request):
    if request.method != 'POST':
        return redirect('bank_charges_filter')
    ...
    # transaction_date is DateField — plain range is correct, no timezone needed
    # recorded_by excluded from select_related — user rows missing from dump
    charges = (
        BankCharge.objects
    ...
    for charge in charges:
        rows.append({
            'date':        charge.transaction_date or charge.expense_date,
            ...
            'hide_date':   (charge.transaction_date == prev_date),
        })
        prev_date = charge.transaction_date

    grand_total = sum(r['amount'] for r in rows)

    return render(request, 'app/bank_charges_report.html', {
        **base_ctx,
        'rows':              rows,
        'grand_total':       grand_total,
        'date_from_display': date_from_display,
...
def completed_loans_approval(request):
    base_ctx        = get_base_context(request)
    filter_office   = base_ctx['filter_office']
    selected_office = base_ctx['selected_office']
    branch_name     = selected_office.name.upper() if selected_office else 'All Branches'

    # Only loans that are fully paid but NOT yet approved
    loans_qs = LoanApplication.objects.filter(
        repayment_amount_remaining__lte=Decimal('0'),
        is_approved=False,
    ...
    # Group by the month the loan was completed (last repayment date)
    # We use the most recent repayment's repayment_date as "completion date"
    loan_ids = [l.id for l in loans_qs]

    # Get last repayment date per loan
    last_repayments = (
        LoanRepayment.objects
        .filter(loan_application_id__in=loan_ids)
        .values('loan_application_id')
        .annotate(
            last_date=models.Max('repayment_date'),
            total_paid=models.Sum('repayment_amount'),
        )
    )
    last_rep_map = {
        ...
        for r in last_repayments
    }

    # Group loans by completion month
    groups = {}
    for loan in loans_qs:
        info      = last_rep_map.get(loan.id, {})
        last_date = info.get('last_date')
        if not last_date:
            # fallback: use application_date
            last_date = loan.application_date

        key   = (last_date.year, last_date.month)
        label = last_date.strftime('%B %Y').upper()
        groups.setdefault(key, {'label': label, 'loans': []})
        groups[key]['loans'].append((loan, info))

    # Build sections sorted newest first
    sections = []
    for key in sorted(groups.keys(), reverse=True):
        grp   = groups[key]
        rows  = []
        total_loaned = total_repayment = total_paid_sum = Decimal('0')

        for loan, info in grp['loans']:
            client     = loan.client
            total_paid = info.get('total_paid', Decimal('0'))
            last_date  = info.get('last_date')
            ...
            rows.append({
                ...
                'interest':        loan.total_interest_amount or Decimal('0'),
                'total_repayment': loan.total_repayment_amount or Decimal('0'),
                'total_paid':      total_paid,
                'completion_date': last_date,
                'loan_date':       loan.application_date,
                'months':          loan.payment_period_months or 0,
                'office':          loan.office or '—',
            })

            total_loaned     += loan.loan_amount            or Decimal('0')
            total_repayment  += loan.total_repayment_amount or Decimal('0')
    ...
    return render(request, 'app/completed_loans_approval.html', {
        **base_ctx,
        'branch_name': branch_name,
        'sections':    sections,
    })
...
def completed_loans_approve(request):
    if request.method == 'POST':
        loan_ids = request.POST.getlist('loan_ids')
        if loan_ids:
            approved = LoanApplication.objects.filter(
            ...
            if approved:
                messages.success(request, f'✅ {approved} loan(s) approved successfully.')
            else:
                messages.warning(request, 'No eligible loans were approved.')
        else:
            messages.warning(request, 'No loans were selected.')
    return redirect('completed_loans_approval')
...
def balance_sheet_filter(request):
    """Page 1: Date picker for Balance Sheet."""
    today = datetime.date.today()
    context = {
        **get_base_context(request),
        'today': today,
    }
    return render(request, 'app/balance_sheet_filter.html', context)
...
def trial_balance_filter(request):
    """Page 1: Date range picker for Trial Balance."""
    today = datetime.date.today()
    default_start = today.replace(month=1, day=1)   # start of current year
    context = {
        **get_base_context(request),
        'today':         today,
        'default_start': default_start,
    }
    return render(request, 'app/trial_balance_filter.html', context)
...
def balance_sheet_report(request):
    ...
    The fix: Retained Earnings is computed as:
        Total Revenue (interest earned on loans)
        - Total Expenses
        - Total Bank Charges
        + Net inter-office transfers received

    Loan disbursements are NOT expenses — they convert Cash → Receivables,
    so they don't affect equity. Only the INTEREST portion of repayments
    is income; the principal portion just reduces receivables.
    """
    from django.utils import timezone
    ...
    end_dt = timezone.make_aware(
        datetime.datetime.combine(as_of_date, datetime.time.max)
    )
    ...
    # CASH IN OFFICE
    # ═══════════════════════════════════════════════════════════════
    cash_rep = rep_qs(upto_q).filter(
        transaction_method__iexact='cash'
    ).aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')

    cash_nyo = nyo_qs(upto_q).filter(
        deposit_method__iexact='cash'
    ).aggregate(t=Sum('amount'))['t'] or Decimal('0')

    cash_to_bank = bank_cash_txn_qs(upto_q).filter(
        source__iexact='cash', destination__iexact='bank'
    ).aggregate(t=Sum('amount'))['t'] or Decimal('0')

    bank_to_cash = bank_cash_txn_qs(upto_q).filter(
        source__iexact='bank', destination__iexact='cash'
    ).aggregate(t=Sum('amount'))['t'] or Decimal('0')

    cash_exp = exp_qs(upto_q).filter(
        payment_method__iexact='cash'
    ).aggregate(t=Sum('amount'))['t'] or Decimal('0')

    cash_loan = loan_qs(upto_q).filter(
        transaction_method__iexact='cash'
    ).aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')

    cash_charge = bank_charge_qs(upto_q).filter(
        payment_method__iexact='cash'
    ).aggregate(t=Sum('amount'))['t'] or Decimal('0')

    cash_in_office = (
        cash_rep + cash_nyo + bank_to_cash
    ) - (
        cash_exp + cash_loan + cash_charge + cash_to_bank
    )

    # ═══════════════════════════════════════════════════════════════
    # CASH IN BANK
    # ═══════════════════════════════════════════════════════════════
    bank_rep = rep_qs(upto_q).filter(
        transaction_method__iexact='bank'
    ).aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')

    bank_nyo = nyo_qs(upto_q).filter(
        deposit_method__iexact='bank'
    ).aggregate(t=Sum('amount'))['t'] or Decimal('0')

    bank_transfer_in = transfer_in_qs(upto_q).aggregate(
        t=Sum('amount')
    )['t'] or Decimal('0')

    bank_exp = exp_qs(upto_q).filter(
        payment_method__iexact='bank'
    ).aggregate(t=Sum('amount'))['t'] or Decimal('0')

    bank_loan = loan_qs(upto_q).filter(
        transaction_method__iexact='bank'
    ).aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')

    bank_transfer_out = transfer_out_qs(upto_q).aggregate(
        t=Sum('amount')
    )['t'] or Decimal('0')

    bank_charge_total = bank_charge_qs(upto_q).filter(
        payment_method__iexact='bank'
    ).aggregate(t=Sum('amount'))['t'] or Decimal('0')
    ...
    # ═══════════════════════════════════════════════════════════════
    # RECEIVABLES (Outstanding Loans — what clients still owe us)
    # ═══════════════════════════════════════════════════════════════
    receivables_qs = LoanApplication.objects.filter(
        created_at__lte=end_dt,
        repayment_amount_remaining__gt=0,
    )
    if filter_office:
    ...
    receivables = receivables_qs.aggregate(
        t=Sum('repayment_amount_remaining')
    )['t'] or Decimal('0')
    ...
    # ═══════════════════════════════════════════════════════════════
    # OWNER CAPITAL & OPENING EQUITY
    # Nyongeza ni capital injection from owner — sehemu ya equity
    # ═══════════════════════════════════════════════════════════════
    owner_capital = Decimal('0')   # leave as 0 unless you add a separate Capital model
    opening_balance_equity = nyo_qs(upto_q).aggregate(
        t=Sum('amount')
    )['t'] or Decimal('0')

    # ═══════════════════════════════════════════════════════════════
    # RETAINED EARNINGS — CORRECTED CALCULATION
    # ═══════════════════════════════════════════════════════════════
    ...
    #
    # Therefore:
    #   RetainedEarnings = (Cash + Bank + Receivables) - OwnerCapital - Nyongeza
    #
    # This automatically gives us the correct profit/loss accumulated
    # because every transaction that changes assets must also change equity
    # (income increases equity, expenses decrease it, loans just shift between
    # asset types so they net to zero).
    # ═══════════════════════════════════════════════════════════════
    ...
    return render(request, 'app/balance_sheet_report.html', context)


# ============================================================================
# TRIAL BALANCE — FIXED VERSION
# ============================================================================
...
def trial_balance_report(request):
    ...
    accounts.append({
        ...
        'opening': opening_cash, 'debit': cash_debit,
        'credit': cash_credit,   'closing': closing_cash,
        'is_credit_balance': closing_cash < 0,
    ...
    accounts.append({
        'code': '1050', 'name': 'Bank', 'type': 'Asset',
        'opening': opening_bank, 'debit': bank_debit,
        'credit': bank_credit,   'closing': closing_bank,
        'is_credit_balance': closing_bank < 0,
        'normal_balance': 'debit',
    })
    ...
    accounts.append({
        ...
        'opening': opening_recv, 'debit': recv_debit,
        'credit': recv_credit,   'closing': closing_recv,
        'is_credit_balance': closing_recv < 0,
        'normal_balance': 'debit',
    })
    ...
    accounts.append({
        ...
        'opening': open_equity, 'debit': equity_debit,
        'credit': equity_credit, 'closing': closing_equity,
    ...
    accounts.append({
        ...
        'opening': opening_retained, 'debit': re_debit,
        'credit': re_credit, 'closing': closing_retained,
        'is_credit_balance': True,
        'normal_balance': 'credit',
    })
    ...
    accounts.append({
        ...
        'opening': Decimal('0'), 'debit': exp_debit,
        'credit': exp_credit, 'closing': closing_exp,
    ...
    accounts.append({
        ...
        'opening': Decimal('0'), 'debit': bc_debit,
        'credit': bc_credit, 'closing': closing_bc,
        'is_credit_balance': False,
        'normal_balance': 'debit',
    })
...
def balance_sheet_export_excel(request):
    ...
    # Reuse the report logic by calling it internally and grabbing the context
    # Quick way: re-run the same calculation
    response = balance_sheet_report(request)
    
    # Easier approach: re-fetch the data
    base_ctx        = get_base_context(request)
    filter_office   = base_ctx['filter_office']
    selected_office = base_ctx['selected_office']
    branch_name     = selected_office.name.upper() if selected_office else 'ALL BRANCHES'
    ...
    # Use the same view to render then extract — instead, recompute quickly
    # For simplicity, redirect and let user use browser print-to-pdf
    # Or: use a request to get the report and parse
    
    # Simpler: just rebuild Excel from a fresh call
    ...
    ws['A1'] = f'BALANCE SHEET — {branch_name}'
    ws['A1'].font = Font(bold=True, size=16, color='FFFFFF')
    ws['A1'].fill = PatternFill(fill_type='solid', fgColor='0d4a7a')
    ...
    # Recompute balance sheet (same logic as the view)
    from django.utils import timezone
    ...
    def rep_qs(q):
        qs = LoanRepayment.objects.filter(q)
        if filter_office: qs = qs.filter(loan_application__office__iexact=filter_office.name)
        return qs
    def loan_qs(q):
        qs = LoanApplication.objects.filter(q)
        if filter_office: qs = qs.filter(office__iexact=filter_office.name)
        return qs
    def nyo_qs(q):
        qs = Nyongeza.objects.filter(q)
        if filter_office: qs = qs.filter(Office=filter_office)
        return qs
    def exp_qs(q):
        qs = Expense.objects.filter(q)
        if filter_office: qs = qs.filter(office__iexact=filter_office.name)
    ...
    def bank_cash_txn_qs(q):
        qs = BankCashTransaction.objects.filter(q)
        if filter_office: qs = qs.filter(office_from=filter_office)
        return qs
    def transfer_in_qs(q):
        qs = OfficeTransaction.objects.filter(q)
        if filter_office: qs = qs.filter(office_to=filter_office)
        return qs
    def transfer_out_qs(q):
        qs = OfficeTransaction.objects.filter(q)
        if filter_office: qs = qs.filter(office_from=filter_office)
    ...
    cash_in_office = (
        (rep_qs(upto_q).filter(transaction_method__iexact='cash').aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')) +
        (nyo_qs(upto_q).filter(deposit_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')) +
        bank_to_cash
    ) - (
        (exp_qs(upto_q).filter(payment_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')) +
        (loan_qs(upto_q).filter(transaction_method__iexact='cash').aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')) +
        (bank_charge_qs(upto_q).filter(payment_method__iexact='cash').aggregate(t=Sum('amount'))['t'] or Decimal('0')) +
        cash_to_bank
    )
    ...
    cash_in_bank = (
        (rep_qs(upto_q).filter(transaction_method__iexact='bank').aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')) +
        (nyo_qs(upto_q).filter(deposit_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')) +
        bank_transfer_in + cash_to_bank
    ) - (
        (exp_qs(upto_q).filter(payment_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')) +
        (loan_qs(upto_q).filter(transaction_method__iexact='bank').aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')) +
        bank_transfer_out +
        (bank_charge_qs(upto_q).filter(payment_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')) +
        bank_to_cash
    )
    ...
    total_assets = cash_in_office + cash_in_bank + receivables

    opening_balance_equity = nyo_qs(upto_q).aggregate(t=Sum('amount'))['t'] or Decimal('0')
    total_income = rep_qs(upto_q).aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')
    total_loans_disbursed = loan_qs(upto_q).aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')
    total_expenses = exp_qs(upto_q).aggregate(t=Sum('amount'))['t'] or Decimal('0')
    total_bank_charges = bank_charge_qs(upto_q).aggregate(t=Sum('amount'))['t'] or Decimal('0')
    net_transfers = bank_transfer_in - bank_transfer_out
    retained_earnings = (total_income + net_transfers) - (total_loans_disbursed + total_expenses + total_bank_charges)
    total_equity = opening_balance_equity + retained_earnings

    # ── Build sheet ────────────────────────────────────────────────
    header_fill = PatternFill(fill_type='solid', fgColor='dcfce7')
    header_font = Font(bold=True, size=12, color='15803d')
    label_font  = Font(size=11)
    value_font  = Font(bold=True, size=11)
    total_fill  = PatternFill(fill_type='solid', fgColor='bbf7d0')
    total_font  = Font(bold=True, size=12, color='14532d')
    thin        = Border(
    ...
    for label, value in [
        ...
        ws.cell(row=row, column=2, value=float(value)).font = value_font
    ...
    ws.cell(row=row, column=2, value=float(total_assets)).font = total_font
    ws.cell(row=row, column=2).fill = total_fill
    ws.cell(row=row, column=2).number_format = '#,##0.00'
    row += 3

    # LIABILITIES & EQUITY
    ws.cell(row=row, column=1, value='LIABILITIES & EQUITY').font = header_font
    ws.cell(row=row, column=1).fill = header_fill
    ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=2)
    row += 1

    for label, value in [
        ('Owner Capital', Decimal('0')),
    ...
    ws.cell(row=row, column=1, value='TOTAL LIABILITIES + EQUITY').font = total_font
    ws.cell(row=row, column=1).fill = total_fill
    ws.cell(row=row, column=2, value=float(total_equity)).font = total_font
    ws.cell(row=row, column=2).fill = total_fill
    ws.cell(row=row, column=2).number_format = '#,##0.00'
...
def trial_balance_export_excel(request):
    ...
    # Call the report view internally to get accounts data
    # Quick way: rebuild a mock request and extract context
    # For Excel we just recompute key totals
    ...
    ws['A1'] = f'TRIAL BALANCE — {branch_name}'
    ws['A1'].font = Font(bold=True, size=16, color='FFFFFF')
    ws['A1'].fill = PatternFill(fill_type='solid', fgColor='0d4a7a')
    ws['A1'].alignment = Alignment(horizontal='center', vertical='center')
    ws.row_dimensions[1].height = 30
    ...
    # Column headers
    headers = ['Code', 'Account Name', 'Type', 'Opening Balance', 'Period Debit', 'Period Credit', 'Closing Balance']
    header_font = Font(bold=True, color='FFFFFF', size=11)
    ...
    # Re-fetch accounts data by calling the inner logic
    # Simplest: just inline what we need
    from django.http import HttpRequest
    
    # Quick: render the report view but as data
    # We'll just call the same calculations
    
    # Build a fake request and call the view
    response = trial_balance_report(request)
    # The context isn't available from response — better to refactor
    # Instead, recompute here (this is short version using simplified calcs)
    
    # For brevity: use a helper or copy/paste the account building logic
    # The cleanest way is to call the view's logic — but we'll skip detail and write basic rows
    
    # Quick recompute (mirror of trial_balance_report)
    ...
    # Build minimal accounts list
    accounts = [
        {
            'code': '1010', 'name': 'Cash', 'type': 'Asset',
            'opening': Decimal('0'), 'debit': Decimal('0'),
            'credit': Decimal('0'), 'closing': Decimal('0'),
        },
        {
            'code': '1050', 'name': 'Bank', 'type': 'Asset',
            'opening': Decimal('0'),
            'debit': (rep_qs(period_q).filter(transaction_method__iexact='bank').aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0')) +
                     (nyo_qs(period_q).filter(deposit_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')),
            'credit': (exp_qs(period_q).filter(payment_method__iexact='bank').aggregate(t=Sum('amount'))['t'] or Decimal('0')) +
                      (loan_qs(period_q).filter(transaction_method__iexact='bank').aggregate(t=Sum('loan_amount'))['t'] or Decimal('0')),
            'closing': Decimal('0'),
        },
        {
            'code': '1100', 'name': 'Receivables', 'type': 'Asset',
            'opening': Decimal('0'),
            'debit':  loan_qs(period_q).aggregate(t=Sum('total_repayment_amount'))['t'] or Decimal('0'),
            'credit': rep_qs(period_q).aggregate(t=Sum('repayment_amount'))['t'] or Decimal('0'),
            'closing': Decimal('0'),
        },
        {
            'code': '3000', 'name': 'Opening Balance Equity', 'type': 'Equity',
            'opening': Decimal('0'),
            'debit':  Decimal('0'),
            'credit': nyo_qs(period_q).aggregate(t=Sum('amount'))['t'] or Decimal('0'),
            'closing': Decimal('0'),
        },
        {
            'code': '5000', 'name': 'Expenses', 'type': 'Expense',
            'opening': Decimal('0'),
            'debit':  exp_qs(period_q).aggregate(t=Sum('amount'))['t'] or Decimal('0'),
            'credit': Decimal('0'),
            'closing': Decimal('0'),
        },
        {
            'code': '5100', 'name': 'Bank Charges', 'type': 'Expense',
            'opening': Decimal('0'),
            'debit':  bank_charge_qs(period_q).aggregate(t=Sum('amount'))['t'] or Decimal('0'),
            'credit': Decimal('0'),
            'closing': Decimal('0'),
        },
    ]
    ...
    for acc in accounts:
        ...
        ws.cell(row=row, column=3, value=acc['type'])
        ws.cell(row=row, column=4, value=float(acc['opening']))
        ws.cell(row=row, column=5, value=float(acc['debit']))
        ws.cell(row=row, column=6, value=float(acc['credit']))
        ws.cell(row=row, column=7, value=float(acc['closing']))
    ...
    ws.cell(row=row, column=1, value='TOTALS').font = total_font
    ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=4)
    ws.cell(row=row, column=5, value=float(total_debit)).font = total_font
    ws.cell(row=row, column=6, value=float(total_credit)).font = total_font
    for col in range(1, 8):
    ...
    # Column widths
    widths = [10, 28, 14, 18, 18, 18, 20]
    for col_num, w in enumerate(widths, 1):
        ws.column_dimensions[ws.cell(row=1, column=col_num).column_letter].width = w
    ...
    response['Content-Disposition'] = f'attachment; filename="trial_balance_{start_date}_{end_date}.xlsx"'
    return response